...
- `docs7/` is a generated Mintlify-compatible mirror of `app/docs` for Docs7 (context7.com/docs7), which hosts it and refreshes the Context7 library on every deploy. The `docs7-mirror` workflow regenerates it on every PR and commits the result onto the PR branch, so page authors do not run anything; pull before pushing again after the bot commit. `pnpm build-docs7` exists for local preview. Never edit `docs7/` by hand. The generator only accepts markup Mintlify renders: string props, no `className`, and the component set listed in `scripts/build-docs7.mjs`.
- `lib/site.ts` is the single source for the public site URL and MCP identity; `docs.config.tsx` and the `.well-known` route handlers derive from it. The `capabilities` block in the server-card route must match what the live `/mcp` server advertises in its initialize response.
