...
This page is the language-agnostic reference for the Limrun REST API. Each operation is shown with code-group tabs for cURL, the three official SDKs (TypeScript, Python, Go), and the `lim` CLI.
...
Assets use `getOrCreate` (upsert) and `getOrUpload` (upsert plus actual upload) instead of `create`. The `os` field on `Asset` is optional; leaving it unset means the asset is available for both platforms. The **`getOrUpload` helper** that handles MD5 dedup and the signed PUT ships in the TypeScript and Go SDKs; Python callers do the two-step pattern manually (call `getOrCreate`, then PUT the bytes to `signedUploadUrl` if `md5` is missing or doesn't match).

<CodeGroup labels={["cURL","TypeScript","Python","Go","npm CLI"]}>
```bash
