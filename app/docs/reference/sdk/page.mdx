...
To narrow that result to instances that started at or before the window start, add `startedAt`:
...
The REST endpoints are `GET /v1/analytics` and `GET /v1/analytics/instances`. Encode nested bounds as query parameters such as `startedAt[lte]` and `stoppedAt[gte]`. At least one time bound is required; one-sided bounds are supported.

Existing top-level `from` and `to` parameters still mean `startedAt >= from` and `startedAt < to`. They intersect with any new bounds. Responses echo those legacy parameters when supplied. Time buckets remain grouped by instance start time. A bounded start-time range, supplied through `startedAt.gte` and `startedAt.lte` or legacy parameters, includes empty buckets. Queries without a bounded start-time range return only populated buckets.
