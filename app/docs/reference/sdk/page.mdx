...
The org API key must never ship to a browser or end-user client. For embedded simulators, your backend holds the API key and passes only `endpointWebSocketUrl` + `status.streamToken` to `<RemoteControl />`. See [Embed the simulator](/docs/platform/embed-simulator#authentication).
...
|---|---|---|---|---|
| `token` | ✓ | ✓ | ✓ | Per-instance bearer for `apiUrl`, `endpointWebSocketUrl`, `adbWebSocketUrl`; embedded in `signedStreamUrl`. Grants every action; keep it server-side. |
| `streamToken` | ✓ |   |   | Browser-only bearer for `endpointWebSocketUrl`: grants the screen and nothing else. Pass it to `<RemoteControl />`. Android returns it in a following release. |
...
| `endpointWebSocketUrl` | ✓ | ✓ |   | WebSocket for `<RemoteControl />`; pass `?token=` with `status.streamToken` from a browser, or `status.token` from your backend. |
