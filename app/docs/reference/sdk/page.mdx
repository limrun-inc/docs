...
Three credentials show up in SDK integrations.

| Credential | Where you get it | Use for |
|---|---|---|
| **Org API key** (`lim_...`) | [console.limrun.com](https://console.limrun.com) → Settings → API Keys; `lim login`; or `LIM_API_KEY` | REST control plane (`api.limrun.com`), including create/list/delete. |
| **Per-instance token** (`instance.status.token`) | Returned in `status` when the instance is `ready` | `apiUrl` device daemon, the per-instance MCP server (`mcpUrl`), `endpointWebSocketUrl` (append `?token=`), `adbWebSocketUrl`, and SDK helpers like `Ios.createInstanceClient`. Embedded in `signedStreamUrl` for browser streaming. |
| **Scoped registry token** (`lim_st_...`) | Minted by your backend with `scopedTokens.create` | Browser connections to the device-install and Apple relay endpoints. Its scopes limit which endpoints and assets the browser can use. |
...
For device installation or Apple account flows in a browser, mint a scoped registry token on your backend:
...
Mint short-lived tokens and hand them to the browser only when it starts a registry session. If one leaks before it expires, revoke it with the `id` returned alongside the token:
...
The three SDKs are not equivalent. Every one covers the control plane (create / get / list / delete instances and assets). Beyond that, only the TypeScript SDK is complete; the others vary. The CLI is the easiest fallback for anything your SDK doesn't cover. Raw HTTP covers REST surfaces, but starting a destination tunnel requires the CLI, TypeScript SDK, or an implementation of its multiplexed WebSocket protocol.

| Capability | TypeScript | Python | Go |
|---|---|---|---|
| Instance CRUD (iOS, Android, Xcode) | ✓ | iOS + Android only (no `xcode_instances` resource) | iOS + Android only (no `XcodeInstances` service) |
| Asset CRUD | ✓ | ✓ | ✓ |
...
| iOS destination tunnel | ✓ (`startTunnel`, `getTunnelStatus`, `stopTunnel`) | not supported | not supported |
...
| iOS camera simulation from a video file | ✓ (`setCameraVideo`) | not supported | not supported |
...
Use `wait: true` for interactive workflows where you need the URLs immediately. Use `reuseIfExists: true` whenever you want repeated calls (CLI re-runs, retries, the same PR's CI runs) to converge on the same instance. Placement follows the caller's location and capacity, so a call from a different network can land in another region and start a fresh instance.
...
|---|---|---|---|---|
| `token` | ✓ | ✓ | ✓ | Per-instance bearer for `apiUrl`, `endpointWebSocketUrl`, `adbWebSocketUrl`; embedded in `signedStreamUrl`. |
| `apiUrl` | ✓ | ✓ | ✓ | HTTP base for the per-instance device daemon (SDK / CLI control client). |
| `signedStreamUrl` | ✓ | ✓ |   | Browser watch URL with the token in the URL fragment, so it never reaches a server (no org API key in the browser). |
| `endpointWebSocketUrl` | ✓ | ✓ |   | WebSocket for `<RemoteControl />`; pass `?token=<status.token>`. |
| `mcpUrl` | ✓ | ✓ |   | Per-instance MCP HTTP endpoint. Auth with `status.token` as `Authorization: Bearer`, not your org API key. See [MCP](/docs/agents/mcp). |
...
| `sandbox.xcode.url` | ✓ (legacy) |   |   | Embedded Xcode sandbox URL on instances created by old clients with `spec.sandbox.xcode.enabled`. New clients create the Xcode instance separately and attach the simulator. |
| `sandbox.playwrightAndroid.url` |   | ✓ (conditional) |   | CDP URL when `spec.sandbox.playwrightAndroid.enabled: true`. |

## Regions and labels

`spec.jurisdiction` (`us`, `eu`, or `as`) keeps an instance inside one jurisdiction. It is a hard constraint: creation fails when no region there has capacity. `spec.region` is deprecated. It is a preference (e.g. `eu-north1`, `us-east1`) that is tried first and overflows to other regions by proximity when the preferred ones are full. If both are omitted, Limrun schedules based on `spec.clues` and current availability. `jurisdiction` is available in the TypeScript SDK and the CLI today; the Python and Go SDKs expose only `region`. The console's **Analytics** tab breaks down runtime minutes per platform and per region, which is the canonical place to see which regions an org has used:
...
When creating an Android, iOS, Xcode, or Gradle instance through the REST API, supply the end user's coordinates as a `ClientLocation` clue to prefer a nearby region:
...
`ClientLocation` takes precedence over `ClientIP` regardless of their order in `spec.clues`. If coordinates are unavailable, pass `{ "kind": "ClientIP", "clientIp": "203.0.113.42" }` to locate the end user by IP. For requests without either clue, Limrun first uses available visitor coordinates, then the connecting IP.

Location clues express a preference. `spec.jurisdiction` still constrains placement, and a full region can cause creation in another eligible region. A specific `spec.region` overrides location clues. Use the REST payload above if your installed SDK does not yet expose `ClientLocation`.

## Analytics time filters

Use `client.analytics.get()` for usage totals or `client.analytics.getInstances()` for per-instance billing details. Instances appear after billing is recorded.

`startedAt` and `stoppedAt` accept independent RFC3339 timestamp bounds. All supplied bounds apply together:

| Bound | Meaning |
|---|---|
| `after` | Strictly after the timestamp. |
| `before` | Strictly before the timestamp. |
...
To narrow that result to instances that started before the window, add `startedAt`:

```ts
const carriedOver = await client.analytics.getInstances({
  startedAt: { before: "2026-09-25T00:00:00Z" },
  stoppedAt: {
    after: "2026-09-25T00:00:00Z",
    before: "2026-09-26T00:00:00Z",
  },
...
The REST endpoints are `GET /v1/analytics` and `GET /v1/analytics/instances`. Encode nested bounds as query parameters such as `startedAt[before]` and `stoppedAt[after]`. At least one time bound is required; one-sided bounds are supported.

Existing top-level `from` and `to` parameters still mean `startedAt >= from` and `startedAt < to`. They intersect with any new bounds. Responses echo those legacy parameters when supplied. Time buckets remain grouped by instance start time. A bounded start-time range, supplied through `startedAt.after` and `startedAt.before` or legacy parameters, includes empty buckets. Queries without a bounded start-time range return only populated buckets.

Historical `stoppedAt` values use the time billing was recorded as an approximation. Billing can be recorded after termination. For periodic imports, rescan overlapping windows and upsert results by `instanceTid`. Keep a reconciliation scan of older windows to catch bills delayed beyond the overlap.
...
The TypeScript SDK exposes a top-level `xcodeInstances` resource with the same CRUD shape as iOS, plus `createClient` for sync, builds and Xcode selection: `xcode.setXcode('27')` binds the Xcode 27 GA release, `xcode.setXcode('27.1')` pins a beta, and every entry in `xcode.getXcode().installed` carries a `channel` of `'ga'` or `'beta'` (see [Choose the Xcode version](/docs/ios/build-with-xcode#choose-the-xcode-version)). The **Python and Go SDKs do not** ship an Xcode instances resource today; reach the same instances through one of these paths:

- Call `POST /v1/xcode_instances` directly over HTTP.
- Or shell out to `lim xcode create` / `lim xcode build`.

To run a build on a simulator, attach the simulator to the Xcode instance after creating both; see [Build with remote Xcode](/docs/ios/build-with-xcode#auto-install-on-the-simulator).

### Assets

Assets use `getOrCreate` (upsert; named `GetOrNew` in Go) and `getOrUpload` (upsert plus actual upload) instead of `create`. The `platform` field on `Asset` is optional; leaving it unset means the asset is available for all platforms. The **`getOrUpload` helper** that handles MD5 dedup and the signed PUT ships in the TypeScript and Go SDKs; Python callers do the two-step pattern manually (call `get_or_create`, then PUT the bytes to `signedUploadUrl` if `md5` is missing or doesn't match).
