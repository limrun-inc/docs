...
To pin an instance to a region (e.g. `eu-north1`, `us-east1`), set the reserved `limrun.com/zone-override` label in `metadata.labels`: the instance is created in that region or not at all (503 when it has no capacity). `spec.region` is deprecated and only a preference; the request may fall back to other regions when the preferred one is full. If neither is given, Limrun schedules based on `spec.clues` and current availability. The console's **Analytics** tab breaks down runtime minutes per platform and per region, which is the canonical place to see which regions an org has used:
...
const instance = await lim.iosInstances.create({
  wait: true,
  reuseIfExists: true,
  metadata: { labels: { session: 'demo', 'limrun.com/zone-override': 'us-east1' } },
  spec: {
    model: 'iphone',
    forceBundleId: 'com.example.myapp',
    sandbox: { xcode: { enabled: true } },
...
lim ios create --reuse-if-exists --xcode \
  --model iphone --label limrun.com/zone-override=us-east1 \
