...
When creating an Android, iOS, Xcode, or Gradle instance, supply the end user's coordinates as a `ClientLocation` clue to prefer a nearby region:

<CodeGroup labels={["TypeScript", "REST payload"]}>
```ts
...
Location clues express a preference. `spec.jurisdiction` still constrains placement, and a full region can cause creation in another eligible region. A specific `spec.region` overrides location clues. If your installed SDK does not yet expose `ClientLocation`, use the REST payload above.
