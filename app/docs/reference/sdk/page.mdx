...
The token inherits the API key's organization. `ttlSeconds` defaults to 3600 and accepts values from 1 through 14400. Scoped tokens cannot be revoked, so hand them to the browser only when it starts a registry session.
