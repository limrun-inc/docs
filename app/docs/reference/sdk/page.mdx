...
Existing top-level `from` and `to` parameters still mean `startedAt >= from` and `startedAt < to`. They intersect with any new bounds. Responses echo those legacy parameters when supplied. Time buckets remain grouped by instance start time. Without both legacy parameters, the response includes only populated buckets.
