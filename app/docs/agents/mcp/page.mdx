...
- **One server per instance.** The instance returns its MCP URL once it's ready (covered below). For an org-level endpoint that also creates and deletes instances, use the [remote MCP server](/docs/agents/remote-mcp) instead.
...
| **Auth** | Org API key in env or config, or a [single pinned instance](/docs/agents/cli#hand-one-instance-to-a-sandboxed-agent) with no key | Per-instance `status.token` as `Authorization: Bearer` |
| **Connection** | Fresh WebSocket per command, or a persistent one via `lim session start` | Long-lived MCP session |
| **What it can do** | Everything Limrun does. Full [SDK reference](/docs/reference/sdk). | Device control only. Five tools, listed below. No builds, no asset uploads. |
...
The `touchDown`, `touchMove`, and `touchUp` actions accept optional `x2` and `y2` coordinates for a second finger. Supply both on every touch action in a two-finger gesture. Both fingers share the optional `screenWidth` and `screenHeight` coordinate space. Omit both coordinates for a single finger.

A `tapElement` action whose selector matches nothing in the accessibility tree fails in about a second and aborts the batch. iOS creates list rows lazily, so a below-the-fold row is often absent from the tree; when you expect that, put `scroll` actions before the `tapElement` in the same batch instead of relying on the server to search.
