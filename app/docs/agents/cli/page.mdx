...
| `--xcode-version <major>` | Xcode major to build with, e.g. `27`. Switches the sandbox when it is bound to another major (its DerivedData resets); refused while it is busy. Also on `create`, `test` and `rbe`; `lim xcode list-xcode` shows the choices and `lim xcode set-xcode 27` switches without building. |
