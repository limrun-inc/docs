...
`lim xcode use xcode@27` selects the workspace Xcode major through the existing Xcode selector, just like `lim xcode version set 27`. It switches an existing sandbox or saves the preference for the next one, without syncing or writing mise configuration. You can combine it with mise requests, such as `lim xcode use xcode@27 node@24`. Xcode accepts only a major; `--cwd` applies only to mise tools.
...
| `--xcode-version <major>` | Xcode major for this one command, e.g. `27`, overriding the workspace preference from `lim xcode version set`. Switches the sandbox when it has another major selected (the other version's build cache is invalidated, so the next build starts cold); refused while it is busy. Also on `create`, `test` and `rbe`; `lim xcode version list` shows the choices. |
