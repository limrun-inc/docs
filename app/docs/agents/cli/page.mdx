...
| `--signing-method app-store-connect\|release-testing\|ad-hoc\|debugging` `--team-id ...` | Use Apple cloud signing instead of a p12 and provisioning profiles. Also requires `--asc-key-id`, `--asc-issuer-id`, and `--asc-key`. Distribution methods require access to cloud-managed distribution certificates. |
