...
Declare each exact TCP destination that the simulator app needs as a `host:port` selector:
...
A tunnel accepts up to ten exact TCP selectors. Ports 1–65535 are supported
except 53; CIDRs and UDP are not supported. The selector set is immutable, so
