...
With both the simulator and the Xcode sandbox pinned, a bare `lim xcode attach-simulator` wires them together so every `lim xcode build` auto-installs onto the simulator, still without a key.
