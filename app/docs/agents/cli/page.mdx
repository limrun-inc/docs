...
The command prints one simulator-facing endpoint per route. Query or stop it from another process:
...
Use the printed endpoint inside the simulator. In this release, declaring `localhost:8000` does not make literal simulator `localhost:8000` route through the tunnel.
