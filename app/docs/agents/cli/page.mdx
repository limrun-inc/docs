...
| `--upload-to-testflight` `--asc-key-id ...` `--asc-issuer-id ...` `--asc-key path.p8` | Upload the signed IPA to TestFlight after the build. Requires the signing flags. Omit `--asc-issuer-id` for individual API keys. |
| `--asc-wait-timeout N` | Seconds to watch App Store Connect's processing verdict (default 120, 0 skips, max 1800). |
| `--auto-build-number` | Set the build number to one more than the highest already in App Store Connect (1 for a new app), so repeat uploads never collide on `CFBundleVersion`. Requires `--upload-to-testflight`. |
