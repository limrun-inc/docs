...
`lim xcode use xcode@27` selects the workspace Xcode major through the existing Xcode selector, just like `lim xcode version set 27`. It switches an existing sandbox or saves the preference for the next one, without syncing or writing mise configuration. You can combine it with mise requests, such as `lim xcode use xcode@27 node@24`. Xcode accepts only a major and does not support `--global`; `--cwd` applies only to mise tools.

For other tools, `use` saves and syncs client mise preferences. Install missing versions with `lim xcode run -- mise install`. Each run or build resolves the mise environment once and uses it throughout that operation. Project requests override personal defaults. Most tools follow the requested major; Ruby, Python, Go, Flutter, Dart and pre-1.0 tools follow `major.minor`. Client patch pins and `mise.lock` do not freeze the image's release. For an exact sandbox override, use `lim xcode run -- mise use --pin node@24.5.0`. Image tools are excluded from workspace caches; user installs can be cached after a successful managed build. See [tool selection](/docs/ios/build-with-xcode#select-developer-tools) for the tool list and precedence.
