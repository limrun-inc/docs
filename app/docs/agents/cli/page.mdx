...
| `--xcode-version <major>` | Xcode major for this one command, e.g. `27`, overriding the workspace preference from `lim xcode version set`. Switches the sandbox when it is bound to another major (its DerivedData resets); refused while it is busy. Also on `create`, `test` and `rbe`; `lim xcode version list` shows the choices. |
