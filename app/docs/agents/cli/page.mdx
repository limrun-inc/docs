...
```bash
lim ios create --xcode --json > instance.json
...
### 3. Hand over the Xcode sandbox too, for builds

`lim xcode build` targets the Mac sandbox, which has its own URL. For an iOS instance created with `--xcode`, that's `status.sandbox.xcode.url` with the same `status.token`:

```bash
export LIM_XCODE_INSTANCE_URL=$(jq -r .status.sandbox.xcode.url instance.json)
export LIM_XCODE_INSTANCE_TOKEN=$(jq -r .status.token instance.json)
