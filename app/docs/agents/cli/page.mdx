...
The CLI sends privacy-safe product events such as skill installation, authentication, the `lim run` lifecycle, and command intent. It does not send command arguments, API keys, hostnames, project paths, repository names, logs, or raw errors. To disable this telemetry:

```bash
export LIM_DISABLE_TELEMETRY=1
```
...
| `--upload-to-appstore` `--asc-key-id ...` `--asc-issuer-id ...` `--asc-key path.p8` | Upload the signed IPA to App Store Connect after the build, making it available for TestFlight or App Store distribution. Requires the signing flags. Omit `--asc-issuer-id` for individual API keys. |
