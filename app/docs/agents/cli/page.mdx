...
The command fetches the latest skills from `limrun-inc/skills` and installs them without prompting. By default it targets every agent that already has a skills directory (or all agents when none exists), installs all skills, and uses project scope. Narrow it with `--agents claude|cursor|codex`, `--skills <name>`, or `--scope global`.
...
`--attach` attaches the new simulator to the most recent Xcode build target, installing the latest successful build immediately. Run it after `lim xcode build`. `--reuse-if-exists` returns an existing instance with exactly the same labels instead of provisioning a new one, so retries land back on the same warm instance. The lookup runs in the region that handles the create, and placement follows your location and capacity, so a retry from a different network can miss and start a fresh instance. Scope reuse to a specific task by adding a label (`--label issue=<ID>`); without one, every reuse call in your org matches the same unlabeled instance.
...
| `--reuse-if-exists` | Return an existing instance with exactly the same labels instead of creating a new one. Looked up in the region that handles the create. |
| `--label key=value` | Repeatable. Used by `--reuse-if-exists` and for cleanup. |
| `--jurisdiction us\|eu\|as` | Keep the instance inside one jurisdiction. Hard constraint: creation fails when no region there has capacity. `--region` is deprecated and only a preference. |
...
`lim xcode use xcode@27` selects the workspace Xcode through the existing Xcode selector, just like `lim xcode version set 27`. A major such as `27` binds that major's GA release; a major.minor such as `27.1` pins that exact version, typically a beta. It switches an existing sandbox or saves the preference for the next one, without syncing or writing mise configuration. You can combine it with mise requests, such as `lim xcode use xcode@27 node@24`. `--cwd` applies only to mise tools.

For other tools, `use` saves and syncs project mise preferences. Install missing versions with `lim xcode run -- mise install`. Each run or build resolves the mise environment once and uses it throughout that operation. Project requests override package-manager detection and image defaults. Personal mise configuration on the client is not read or forwarded. Most tools follow the requested major; Ruby, Python, Go, Flutter, Dart and pre-1.0 tools follow `major.minor`. Client patch pins and `mise.lock` do not freeze the image's release. For an exact sandbox override, use `lim xcode run -- mise use --pin node@24.5.0`. Image tools are excluded from workspace caches; user installs can be cached after a successful managed build. See [tool selection](/docs/ios/build-with-xcode#select-developer-tools) for the tool list and precedence.
...
| `--sdk iphonesimulator\|iphoneos\|watchsimulator\|watchos\|appletvos\|xros` | Target SDK. tvOS and visionOS are device-build only. |
| `--xcode-version <major\|major.minor>` | Xcode for this one command: a major such as `27` binds that major's GA release, a major.minor such as `27.1` pins that exact version (typically a beta). Overrides the workspace preference from `lim xcode version set`. Switches the sandbox when it has another Xcode selected (the other version's build cache is invalidated, so the next build starts cold); refused while it is busy. Also on `create`, `test` and `rbe`; `lim xcode version list` shows the choices. |
...
| `--detach` | Return once the build is accepted. Prints a follow-up `lim xcode logs` command; a webhook is optional. |
...
| `--build-setting KEY=VALUE` | Pass an xcodebuild build setting; it replaces a Limrun default with the same key. Repeatable. |
...
Use `--sdk appletvos --scheme <tvOS-scheme>` or `--sdk xros --scheme <visionOS-scheme>` for tvOS and visionOS. These builds produce IPAs but do not support simulator attachment, installation, or XCTest yet.
...
### Detached builds and build logs

Use `--detach` to return after a build is accepted; `--webhook-url` is optional.
Read the latest build on the remembered instance without supplying an exec ID:
...
For Android builds, use `lim gradle build . --detach`, `lim gradle logs`, and
`lim gradle logs --follow`.

`logs` prints a snapshot, including a `RUNNING` status when the build is still
in progress. `--follow` waits for the selected build to finish. Stopping it leaves
the remote build running. Check the reported build status before declaring success.

The CLI reads the daemon's latest retained stream, then falls back to persisted
S3/R2 logs on a 404, including after instance deletion. It never creates a
replacement instance. Other errors are reported without showing an older log.
Use `--id <instance-id>` to select another instance, or copy the detached build's
printed `logsCommand` to keep inspecting that build after newer builds start.
Exec IDs are optional; do not ask the user for one to inspect the latest build.

The SDK's `observeBuildLogs('latest')` reads only the retained daemon stream.
SDK callers fetch persisted logs separately with `listBuildLogs(instanceId)` and
the returned `downloadUrl`. See the [Xcode](/docs/ios/build-with-xcode#build-completion-webhooks-and-detached-builds)
and [Gradle](/docs/android/build-with-gradle#build-completion-webhooks-and-detached-builds) examples.
...
For an iPhone Duo, use `lim ios fold` to read or change the hinge angle, and
`--display inner` or `--display outer` with `screenshot` and `tap` to target a
display. The existing element tree and default screenshot do not automatically
follow the inner display. See [Fold an iPhone Duo](/docs/ios/run-simulator#fold-an-iphone-duo)
for commands and coordinate details.
...
Hardware button names for `buttonDown` and `buttonUp`: `home`, `lock`, `side`, `applePay`, `softwareKeyboard`. Native iPhone Duo also supports `volumeUp` and `volumeDown`.

For two-finger gestures, supply `x2` and `y2` together on every `touchDown`, `touchMove`, and `touchUp`. Both fingers share the optional `screenWidth` and `screenHeight` coordinate space. For example, a two-finger tap:
...
Declare each destination the simulator app needs as a selector: an exact
`host:port`, or a domain (exact or `*.` wildcard) that should be dialed from the
agent machine:
...
`localhost:8000`. Domain selectors are intercepted on the instance and dialed
from the agent machine whether or not the name resolves on public DNS;
wildcards are label-bound (`*.staging.example` does not cover
`staging.example`). Apps that resolve DNS over HTTPS (DoH) bypass domain
interception.

A tunnel accepts up to ten exact TCP selectors and up to 64 domain selectors.
Ports 1 to 65535 are supported except 53; CIDRs and UDP are not supported. The
selector set is immutable, so
stop and restart the tunnel with the complete selector list when destinations
change.
An open idle tunnel does not count as instance activity and does not prevent the
simulator's inactivity timeout.

HTTP and HTTPS through the tunnel are inspected by default: `--har <path>`
writes HAR 1.2, `--persist` keeps a network log on the session, and
`--no-inspect` keeps TLS end to end for apps that pin certificates.

Start the tunnel before launching the app or opening the page. Connections the
app opened before the tunnel was ready can keep bypassing it; relaunch the app
...
<Columns cols={2}>
  ...
  <Card title="Remote MCP server" icon="bot" href="/docs/agents/remote-mcp">
    One org-level endpoint at mcp.limrun.com: agents create, drive, and delete devices with OAuth sign-in, no CLI required.
  </Card>
</Columns>
