...
# Auto-delete the pod when this CLI process exits, keep it in the US, and label it.
...
| `--reuse-if-exists` | Return the existing instance with exactly the same labels instead of creating a new pod. Looked up in the region that handles the create. Defaults to `false`. |
...
| `reuseIfExists` | boolean | When `true`, return the existing instance whose `metadata.labels` match exactly instead of creating a new pod. The lookup runs in the region that handles the create, so a placement change can miss. Defaults to `false`. |
...
| `spec.jurisdiction` | `'us' \| 'eu' \| 'as'` | Hard constraint: schedule only inside this jurisdiction; creation fails when no region there has capacity. TypeScript SDK and CLI today; the Python and Go SDKs expose only `region`. |
| `spec.region` | string | Deprecated. A region preference (e.g. `us-east1`), not a hard pin: the request overflows to other regions by proximity when the preferred ones are full. When omitted, the scheduler picks based on `clues` plus availability. |
...
Set `initialAssets` at provisioning time to install one or more APKs while the pod boots. Each APK must come from a publicly reachable HTTPS URL (a GitHub release asset, an S3 presigned URL) or Limrun's [Asset Storage](/docs/platform/asset-storage). After a successful installation, the app is launched automatically so the instance is ready to interact with on first connect.

For local files, the CLI's `--install` and the TypeScript/Go SDK's `assets.getOrUpload({ path })` upload to Asset Storage first and hand the daemon a signed URL. Python's SDK doesn't ship that helper. Upload manually with `assets.get_or_create` plus a PUT to the returned upload URL before calling `create`. See [Asset Storage](/docs/platform/asset-storage).
...
`lim android sync` uses differential patching to update an APK on a running emulator. It transfers an xdelta3 patch against the APK already available on the instance, then installs the updated APK. This avoids uploading the full APK after every rebuild.
...
To target a specific instance, pass `--id <ID>`. Add `--watch` to keep watching the APK file and sync each new build automatically.
...
```json
{
  ...
  "status": {
    ...
    "signedStreamUrl":      "https://console.limrun.com/signedStream?url=wss%3A%2F%2Fus-west-1-1234567.limrun.net%2F...%2FendpointWebSocket#token=lim_...",
...
| `status.signedStreamUrl` | Console-hosted watch URL with the token in the URL fragment. Open in a browser without your API key. |
| `status.mcpUrl` | Per-instance MCP HTTP endpoint. Authenticate with `status.token`, not your org API key. See [MCP](/docs/agents/mcp); for one org-level endpoint that also creates instances, see the [remote MCP server](/docs/agents/remote-mcp). |
...
### Shell and files without a tunnel

For one-shot shell commands and file transfer, the CLI talks to the instance directly — no local `adb` install and no tunnel to keep alive. Everything runs with the same permissions the ADB shell user has, so protected locations are rejected just as they would be for `adb`.

`adb-shell` runs a command and returns its stdout, stderr, and exit code (the CLI exits with the command's exit code). Pass the command after `--`; each argument is sent as a separate, quoted token, so shell features like pipes need an explicit shell:

```bash
lim android adb-shell -- pm list packages -3
lim android adb-shell -- getprop ro.build.version.sdk
lim android adb-shell --id <ID> -- sh -c "dumpsys battery | grep level"
```
...
```bash
lim android create --record --app-logs com.example.myapp --events --persist-ttl 24h
```

- `--record` persists the full session video as an MP4.
- `--app-logs <packageName>` persists that app's logcat output as timestamped JSON lines (`{"ts": <epoch millis>, "line": "..."}`).
...
```ts
const recordings = await limrun.androidInstances.listRecordings(id);
const appLogs = await limrun.androidInstances.listAppLogs(id);
const events = await limrun.androidInstances.listEvents(id);
