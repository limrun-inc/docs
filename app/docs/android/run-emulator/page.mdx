...
There is a third connection for the app's own traffic: the destination tunnel (`lim android tunnel`) routes selected destinations from the emulator through your machine, so the app reaches your local dev server or VPN-only backend, and you can inspect its HTTP traffic or capture a HAR. See [Network Tunneling](/docs/platform/network-tunnel).
...
<Cards>
  ...
  <Card title="Network Tunneling" href="/docs/platform/network-tunnel">
    Route the app's destinations through your machine and inspect its HTTP traffic.
  </Card>
</Cards>
