...
lim android create --rm --label limrun.com/zone-override=us-east1 --label app=demo --label session=docs
...
const instance = await lim.androidInstances.create({
  ...
  metadata: { labels: { app: 'demo', session: 'docs', 'limrun.com/zone-override': 'us-east1' } },
  spec: {
...
| Flag | Use this to |
|---|---|
| `--label limrun.com/zone-override=<region>` | Pin the instance to a region with no fallback: creation fails with 503 when that region has no capacity. |
| `--region <name>` | Deprecated: a region preference only; may fall back to other regions when full. |
...
| `spec.region` | string | Deprecated: a preference only; may fall back to other regions when full. Pin with the `limrun.com/zone-override` label in `metadata.labels` instead. When omitted, the scheduler picks based on `clues` plus availability. |
