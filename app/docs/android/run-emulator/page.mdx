...
| `spec.clues[].clientIp` | string | Set this to the end user's IP for geographic placement. An explicit nonempty clue takes precedence over Cloudflare's `CF-Connecting-IP` header, which Limrun uses when the clue is missing or empty. Region preferences, jurisdiction, availability, and organization routing policies still apply. |
