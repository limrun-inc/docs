...
dependency cache invalidation, so unused missing tools in a shared mise file
do not force reinstalls or block unrelated commands.
...
The client forwards personal mise tool defaults automatically. `run` accepts
`cwd`, `env`, and `timeoutSeconds` and returns the same streamed process interface
as `gradlebuild`.
