...
- The AAB's `versionCode` has never been uploaded to this app before. Bump `versionCode` in `app/build.gradle(.kts)` (Expo: `expo.android.versionCode` in `app.json`) before the build.
