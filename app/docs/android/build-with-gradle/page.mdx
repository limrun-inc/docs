...
formatting. Use `--cwd apps/mobile` for a nested project. Runs and builds in that
directory automatically install missing selected tools.
...
until the instance is removed. Resolved tool versions control dependency cache
invalidation. Reusing the same versions preserves the dependency cache.

### Commands through the TypeScript SDK

After syncing a project, builds and commands install missing selected tools automatically:
