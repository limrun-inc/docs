...
`lim gradle sync [path]` uploads a project to a Gradle sandbox without running
Gradle or installing dependencies. The path defaults to the current directory.
The command reuses the instance remembered for your worktree or creates one
when needed. Pass `--id` to choose a target.
...
`--watch` keeps pushing source changes until you press Ctrl+C. Each completed
sync reports its duration and bytes sent. Use `--basis-cache-dir` to choose the
local delta cache, `--ignore <regex>` to exclude paths, and `--include <regex>`
to force-sync inputs. Both filters are repeatable.

Upload extra files outside the source tree with repeatable
`--additional-file localPath=remotePath`. Remote paths are relative to the
sandbox workspace:
...
Additional files are included in every sync pass but are not watched directly.
`lim gradle build` already syncs once before building, so it does not require a
separate sync command.

The TypeScript SDK supports the same watch mode:

```ts
const watcher = await gradle.sync('.', {
  watch: true,
  onSyncComplete: ({ bytesSent, durationMs }) => {
    console.log(`Synced ${bytesSent} bytes in ${durationMs}ms`);
  },
});
// Stop watching when the session ends.
...
that override. `lim gradle use node@24` replaces it with a Node 24 selection in the sandbox.
...
Builds and commands preserve the instance's temporary files and dependency
caches. Reuse the same instance for warm builds; shared temporary files remain
until the instance is removed. Only installed tool selections contribute to
dependency cache invalidation. After the first installation attempt, unused missing
tools do not force reinstalls or block unrelated commands.

### Commands through the TypeScript SDK

After syncing a project, use `run` to install tools or execute a command:
...
const result = await gradle.gradlebuild({ env: ['APP_ENV=staging'] });
```

`run` accepts `cwd`, `env`, and `timeoutSeconds` and returns the same streamed
process interface as `gradlebuild`.
...
`logsUrl` is a time-limited link to the persisted plain-text build log. Read that log for build diagnostics when `status` is `FAILED`. There is no `error` field: Gradle doesn't expose one deterministic diagnostic, so the log is the answer. The instance, console, log, timing, and exit-code fields are omitted when they are unavailable.

### App identity in the payload

`applicationId`, `versionName`, and `versionCode` describe the app the build produced. They come from the built APK's or AAB's manifest, not from your Gradle scripts, so they reflect the merged manifest: product flavors, build-type suffixes, and manifest placeholders are all resolved the way the shipped artifact has them.

This is what lets a receiver tell builds apart without downloading and opening the artifact. In a monorepo that ships several apps, key your own bookkeeping on `applicationId`. The iOS equivalents are documented in [Build with remote Xcode](/docs/ios/build-with-xcode#app-identity-in-the-payload).

These fields are set on successful builds that produced an artifact, which means the request also asked for an upload (`--upload` or `--signed-upload-url`) or a Play Store publish. A build that packaged nothing has no artifact to describe. Reading the manifest is best-effort: an artifact Limrun couldn't read leaves the fields out rather than failing a build that packaged and uploaded fine. `versionCode` is omitted when the manifest declares none.
...
```bash
lim gradle build . --detach --inactivity-timeout 3s
```
...
`--webhook-url` is optional; add it to receive the terminal result automatically. `--inactivity-timeout` creates a fresh Gradle instance with the requested lifecycle, so it cannot be combined with `--id`. Active builds count as activity, and the timeout starts expiring only after build and upload work stops.
...
```bash
lim gradle logs build-1776140344112378000 --id <instance-id>
```

The SDK can replay or follow a build while its daemon remains available:

```ts
const result = await gradle.observeBuildLogs('latest', {
  follow: true,
  onEvent: (event) => {
...
```ts
const completedBuilds = await lim.gradleInstances.listBuildLogs(instance.metadata.id);
```

This list remains available after the instance is deleted.
