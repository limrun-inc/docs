...
- The simulator needs to be set up so it accepts a local config. That means **attaching it to an Xcode sandbox** (the same sandbox you use for [Build with remote Xcode](/docs/ios/build-with-xcode)), and ad-hoc-signing the build so the simulator runs it without an Apple Developer account.
...
## Provision an Xcode sandbox and an attached simulator

Create the two instances separately and attach the simulator to the sandbox — the same shape as any build-and-test flow:

<CodeGroup labels={["CLI", "TypeScript", "Python"]}>
```bash
lim xcode create --reuse-if-exists --region us-west --label name=iap-test
lim ios create --attach --reuse-if-exists --region us-west --label name=iap-test
```
...
// you on the same warm instances on re-runs; pin a region if you want that
// to be reliable across networks (see Run an iOS Simulator).
...
# with the Xcode instance's status.token as bearer and body
# {"apiUrl": instance.status.api_url, "token": instance.status.token}.
```

</CodeGroup>

Two URLs matter for IAP work:

- **The Xcode sandbox URL** (`xcodeInstance.status.apiUrl`, token `xcodeInstance.status.token`). The Xcode client connects here to sync source and run `xcodebuild`.
- **The device control-plane URL** (`instance.status.apiUrl`, token `instance.status.token`). The iOS client connects here to call the StoreKit helpers and drive the UI.
