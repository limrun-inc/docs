...
```bash
lim ios create --xcode --reuse-if-exists --label limrun.com/zone-override=us-east1 --label name=iap-test
```
...
const instance = await lim.iosInstances.create({
  wait: true,
  reuseIfExists: true,
  metadata: { labels: { name: 'iap-test', 'limrun.com/zone-override': 'us-east1' } },
  spec: {
