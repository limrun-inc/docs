...
<CodeGroup>
```bash title="CLI"
lim ios create --xcode --reuse-if-exists --region us-east --label name=iap-test
```
...
The three StoreKit helpers below live on the iOS device-control client:
...
```ts
// 'data' (default): terminate the app + wipe its data container. Fast.
...
// from the cached source .app. Takes a few seconds longer.
await ios.softReset(bundleId, { strategy: 'full' });
```

To roll the whole environment back to "no local config," see the next section.
