...
The sandbox installs and selects one version per developer tool with mise before shell commands and managed builds. Set a project preference from your client:
...
`use` saves compatibility lines in the highest-precedence existing `mise.toml`, `.mise.toml`, `mise.local.toml`, or `.mise.local.toml` in the current directory, creating `mise.toml` when none exists. It syncs the project and prepares missing tools remotely. It does not install tools on your client. `--cwd apps/mobile` selects a directory inside the sync root; `--id` selects a sandbox. The command rewrites the selected TOML file, preserving its configuration values but not comments or formatting.
...
Additional tools and compatibility lines are installed through mise when requested. Select another Java vendor with a request such as `java@corretto-17`. Upstream support for macOS and the selected runtime still applies. The managed CocoaPods resolver uses the selected CocoaPods tool; a project `Gemfile` does not select its gems.
...
`mise exec` selects tools for one command. `mise use` records an explicit sandbox override for later operations in `.limrun-runtime-mise.toml`; it takes precedence over client defaults. `lim xcode use node@24` clears that tool's override in the selected directory and returns to the compatibility-line policy. A version switch affects new commands; use `mise exec` when the same shell needs the new version immediately.

Image installations stay outside the workspace cache. A restored user installation cannot replace the image's default release unless an explicit sandbox override selects it. User-installed mise tools live under `.limbuild-sandbox/home/.mise/` and are included when your cache paths cover that directory. Existing cache publication rules still apply: running a command alone does not make a workspace eligible; a successful managed build is required. Dependency installs rerun when the resolved tool versions change. Switching Xcode still invalidates the workspace cache.
