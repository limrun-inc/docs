...
<CodeGroup labels={["CLI","TypeScript"]}>
```bash
lim xcode version list                    # versions the sandbox can build with; notes mark beta N, selected, default, preferred
...
- Switching resets the DerivedData of every synced project, so the next build starts cold. The build cache is keyed by Xcode version too, so a cached workspace produced by another Xcode is not reused.
...
- Asking for a major the node does not have fails with the available list, and `lim xcode version set` does not record a preference for it. Only majors are selectable; the minor is whatever the fleet carries for that major. Beta majors are labeled with Apple's seed number in `lim xcode version list` (`beta 6`).
