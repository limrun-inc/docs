...
Like `--ignore`, the pattern is a regular expression, not gitignore syntax. To reach files inside a directory that is ignored as a whole, the pattern must also match the directory path itself, as both examples do. On the SDK, pass an `include` predicate alongside `ignore`.
