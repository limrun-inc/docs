...
Keep generated files in the synced workspace and temporary files under `$TMPDIR`. Host logs and unrelated host configuration under `/etc` or `/var` are unavailable. Sync project configuration into the workspace instead of depending on host files.
