...
Pass a cache key to restore a saved workspace and publish it under that key when the instance terminates after a successful build:

```bash
lim xcode build . --cache-key=my-app
```

If no archive matches the key, or the saved archive no longer exists, the build continues with a cold workspace. The CLI reports `No cache restored`; this is an expected cache miss. Permission errors and other transfer failures still stop the build.
