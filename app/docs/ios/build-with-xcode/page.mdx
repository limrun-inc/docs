...
|---|---|---|
| `Unknown issuer hash` or `not a valid code-signing identity` | The p12 holds no Apple Distribution or Apple Development certificate with its private key, or its issuing CA isn't recognized because the p12 carries no CA chain. | Re-export the p12 with the certificate, its private key and the certificate chain included (see the OpenSSL command above). |
...
| `no provisioning profile covers ...` or `requires a provisioning profile` | An embedded bundle (extension, watch app) has no matching provisioning profile. | Pass one `--provisioning-profile` per bundle id named in the error. |
