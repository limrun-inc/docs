...
Use `lim xcode run` as the escape hatch when the repository needs another macOS command, such as a Make target or code generator. The command syncs the current directory, then runs after `--` in the remote workspace:
