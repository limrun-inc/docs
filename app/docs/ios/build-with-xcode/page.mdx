...
On top of that, paths matched by your `.gitignore` files are skipped, root and nested ones alike, with git's usual precedence. Two exceptions always sync even when gitignored: `.xcconfig` files, because Xcode needs them on the remote, and `.env` files (`.env`, `.env.local`, `.env.production`, ...), because Expo app configs and build scripts read them at build time. If a file is both tracked by git and ignored, the SDK warns you so you can fix the mismatch.

To set environment variables on the remote build, pass `--env KEY=VALUE` (repeatable). The variables reach every build command — dependency installs, `expo prebuild`, `pod install`, `xcodebuild` and its script phases — but server-managed variables like `PATH` and `HOME` cannot be overridden. Which `.env` variants Expo actually loads depends on `NODE_ENV`, so the CLI also propagates your shell's `NODE_ENV` automatically: with `NODE_ENV=production lim xcode build .` (or `--env NODE_ENV=production`), the remote Expo config evaluation loads `.env.production` the same way a local build would. Dependency installs always include devDependencies, so `NODE_ENV=production` cannot break them.
...
Expo projects need no local prebuild step: the sandbox auto-detects whether the native `ios/` tree needs generating and runs `expo prebuild` server-side before the build. Keep the generated project gitignored as usual; use `--expo-app-dir` when auto-detection can't pick the app in a monorepo.

If detection decides not to prebuild when it should have, force it:
