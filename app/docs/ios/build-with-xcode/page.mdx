...
Build commands cannot directly read or write the host Mac's `/Library/Preferences/com.apple.networkextension.plist` or `com.apple.networkextension.uuidcache.plist`, including through `/System/Volumes/Data`. Keep project settings in your workspace; permission errors for these host files are expected.
