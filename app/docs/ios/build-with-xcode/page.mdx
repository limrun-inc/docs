...
Every upload needs a `CFBundleVersion` higher than the last one for that version. Pass `--auto-build-number` (`autoIncrementBuildNumber: true` in the SDK's `appstore` options) to set it to one more than the highest build number already in App Store Connect, or 1 for a new app. The lookup runs server-side with the same API key used for the upload. It works for projects that use Xcode-standard versioning (`CFBundleVersion = $(CURRENT_PROJECT_VERSION)`), including modern Xcode templates and Expo prebuilds.
