...
Device builds that upload their IPA by asset name additionally carry the app's identity in the payload: `assetId`, `bundleIdentifier`, `shortVersion`, `buildVersion`, `displayName`, `deeplink`, and a time-limited `iconUrl` for the extracted app icon. Each field appears only when the built bundle (or an explicit `--upload-option`) provided it.
...
Uploading by asset name also records the built app's metadata on the asset automatically: the app title, bundle identifier, versions, deep link scheme, and icon, all read from the built bundle. Consumers such as over-the-air installation and the Console use whichever fields are present. To record a field the bundle does not declare, pass the repeatable `--upload-option key=value` flag (CLI) or `upload: { assetName, uploadOptions }` (TypeScript) with any of `displayName`, `bundleIdentifier`, `shortVersion`, `buildVersion`, or `deeplink`; values extracted from the bundle win over explicit ones. See [Asset Storage](/docs/platform/asset-storage) for details.
