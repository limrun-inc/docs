...
On top of that, paths matched by your `.gitignore` files are skipped, root and nested ones alike, with git's usual precedence. Built-in force-includes keep `.xcconfig` and `.env` files in the sync because builds commonly need them on the remote. If a file is both tracked by git and ignored, the SDK warns you so you can fix the mismatch.
...
An explicit CLI `--ignore` takes precedence over built-in force-includes and `--include`. It can exclude `.env` or `.xcconfig` files, and adding a matching `--include` does not restore them. Non-overridable safety exclusions also remain excluded.
