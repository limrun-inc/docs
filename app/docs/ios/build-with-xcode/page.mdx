...
Three rules apply. Every capability must also be enabled on your App ID in the developer portal, or the export fails naming the missing one. Values must be fully expanded: build-setting references like `$(AppIdentifierPrefix)` are rejected, so write the concrete team prefix instead. And keys the export manages itself (`application-identifier`, `com.apple.developer.team-identifier`, `get-task-allow`, `beta-reports-active`) must be omitted; the export sets them from the generated profile and distribution method.
