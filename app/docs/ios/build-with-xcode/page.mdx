...
Build scripts and remote commands must write temporary files under the provided
`TMPDIR`. Host temp paths such as `/tmp`, `/var/tmp`, and `/var/folders` are
inaccessible. If a tool reports `Operation not permitted` at one of these paths,
configure its temp directory under `$TMPDIR`.
