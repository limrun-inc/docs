...
- The switch is refused while a build, command, sync, or `lim xcode rbe` stack is running; stop them first (`lim xcode rbe --stop`). `lim xcode version set` keeps the preference in that case and the next command retries the switch.
- Asking for a major the node does not have fails with the available list, and `lim xcode version set` does not record a preference for it. Only majors are selectable; the minor is whatever the fleet carries for that major. Beta majors show Apple's seed number in `lim xcode version list` (`27.0 beta 6`).
...
`use` saves compatibility lines in the highest-precedence existing `mise.toml`, `.mise.toml`, `mise.local.toml`, or `.mise.local.toml` in the current directory, creating `mise.toml` when none exists. It syncs the project and shows the selected versions. It does not install tools; run `lim xcode run -- mise install` when a selected version is missing. `--cwd apps/mobile` selects a directory inside the sync root; `--id` selects a sandbox. The command rewrites the selected TOML file, preserving its configuration values but not comments or formatting.
...
Homebrew, Xcode, Apple SDKs and simulator runtimes have separate management. Use `lim xcode version` for Xcode. Mise itself and Limrun's runtime helpers are part of the image.
