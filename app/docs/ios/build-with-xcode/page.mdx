...
### Get notified when the build finishes

Builds can outlive the process that triggered them: a CI step hits its own timeout, an agent gets recycled, a bot fires the build and moves on. Instead of holding the log stream open, pass a webhook with the build request. The sandbox calls your endpoint once, when the build reaches a terminal state (`SUCCEEDED`, `FAILED`, or `CANCELLED`).

The build request takes a webhook `url` and optional `headers`. Use a header to authenticate the call to your endpoint:

<CodeGroup labels={["CLI","TypeScript"]}>
```bash
lim xcode build . --webhook-url https://ci.example.com/hooks/limrun-build --webhook-header Authorization="Bearer $HOOK_SECRET"
```
...
const build = xcode.xcodebuild(
  ...
  {
    webhook: {
      url: 'https://ci.example.com/hooks/limrun-build',
...
When the build finishes, the sandbox POSTs a JSON payload to that URL with your headers set:
...
`logsUrl` is a presigned link to the full build log, valid for 24 hours. Fetch it from your webhook handler to attach logs to a PR comment or a failure report without keeping any connection to the sandbox open. On failure the payload also carries an `error` field with the message the build recorded.

`instanceId` is the sandbox instance the build ran on, and `consoleUrl` opens that instance's builds page in the Limrun Console. Put the link in your PR comment or failure notification so whoever picks it up lands directly on the build to debug.

What to know:

- The URL must be HTTPS on a public DNS host. IP literals and private or internal addresses are rejected when you submit the build.
- Up to 16 headers are allowed. `Host`, `Content-Length`, `Transfer-Encoding`, and `Connection` are rejected.
- Any 2xx response from your endpoint counts as delivered. Anything else is retried up to two more times, then dropped. A webhook failure never fails the build.
- Cancelled builds notify too. Triggering a new build cancels the one in flight, so an endpoint that only expects success and failure should handle `CANCELLED` as well.
- Android Gradle builds support the same webhook: pass the same flags to `lim gradle build` (or `webhook` in the TypeScript SDK's `gradlebuild` options). The payload carries `"command": "gradlebuild"` and no `logsUrl`.
