...
Each `lim xcode run` and `lim xcode build` automatically installs missing selected tools and resolves `mise env --json` once at the start. Dependency installation, project generation, builds and their child processes use that environment. Tool selection requires a sandbox image with mise support. Set a project preference:
...
For mise tools, `use` saves compatibility lines in the highest-precedence existing `mise.toml`, `.mise.toml`, `mise.local.toml`, or `.mise.local.toml` in the current directory, creating `mise.toml` when none exists. It syncs the project and shows the selected versions. Sandbox runs and builds automatically install missing selected tools. `--cwd apps/mobile` selects a directory inside the sync root; `--id` selects a sandbox. The command rewrites the selected TOML file, preserving its configuration values but not comments or formatting.
...
Mise installs additional tools and compatibility lines into the sandbox home when resolving a run or build environment. Existing compatible installations are reused. An unavailable tool or failed download stops the operation, even if the command does not use that tool. Fix its project request and sync again to retry. You can still use `lim xcode run -- mise install` explicitly. Select another Java vendor with a request such as `java@corretto-17`. Upstream support for macOS and the selected runtime still applies. The managed CocoaPods resolver uses the selected CocoaPods tool; a project `Gemfile` does not select its gems.
