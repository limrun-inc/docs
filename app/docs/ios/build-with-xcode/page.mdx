...
Python and Go users provision the instance over REST (`POST /v1/xcode_instances`) or with `lim xcode create`, and drive the build with `lim xcode build`. See the [SDK capability matrix](/docs/reference/sdk#sdk-capability-matrix).
...
lim xcode use xcode@27                    # prefer 27 here; switches the remembered sandbox now
...
```ts
const { bound, installed } = await xcode.getXcode();
if (bound.major !== '27') {
...
- The switch is refused while a build, command, sync, or `lim xcode rbe` stack is running; stop them first (`lim xcode rbe --stop`). `lim xcode use xcode@<major>` keeps the preference in that case and the next command retries the switch.
- Asking for a major the node does not have fails with the available list, and `lim xcode use xcode@<major>` does not record a preference for it. Only majors are selectable; the minor is whatever the fleet carries for that major. Beta majors show Apple's seed number in `lim xcode version list` (`27.0 beta 6`).
- Simulators keep running the fleet's default runtime. `lim xcode test` with a non-default Xcode warns and proceeds: XCTest bundles built by Xcode 27 run against the simulator's Xcode 26 frameworks, which works for most suites but is not guaranteed.
- Apple rejects App Store uploads built with a beta Xcode, so keep `--upload-to-appstore` on the default until the GM ships.
...
- The sandbox uses the selected XcodeGen tool. Use `lim xcode use xcodegen@2` to select its compatibility line, or an explicit sandbox mise override when generation requires an exact release.
...
Each `lim xcode run` and `lim xcode build` resolves `mise env --json` once at the start. Dependency installation, project generation, builds and their child processes use that environment. Sync your project, then select tools in the sandbox:
...
`lim xcode use` selects tools in an existing sandbox after project sync. Mise installs a requested version if needed; `lim xcode tools install` installs selections from synced project files. Use `--cwd apps/mobile` for a nested project and `--id` to choose a sandbox. Explicit sandbox selections take precedence over project requests and apply to subsequent commands.

Project tool requests take precedence over the detected package-manager major and Limrun's image defaults. Deeper project files override parent files within the synced directory.

Limrun guarantees compatibility lines, allowing image updates to deliver newer releases within those lines:
...
Limrun imports `[tools]` declarations from synced project mise files. Ruby, Python, Go, Flutter, Dart, and tools below version 1 retain `major.minor`; other numeric versions retain their major. `latest` keeps its mise meaning and opts out of a fixed compatibility line. Other custom selectors should be used through an explicit sandbox mise command.
...
The first sandbox operation that reads a project mise file runs `mise install` before exporting its environment. This happens once per sandbox, even when later files or tool selections change. Without a project mise file, commands keep using the image defaults. A failed or cancelled first installation stops that operation; retry explicitly with `lim xcode tools install`. That command also installs later tool selections. Select another Java vendor with a request such as `java@corretto-17`. Upstream support for macOS and the selected runtime still applies. The managed CocoaPods resolver uses the selected CocoaPods tool; a project `Gemfile` does not select its gems.

Homebrew, Xcode, Apple SDKs and simulator runtimes have separate management. Select Xcode with `lim xcode use xcode@27` and inspect it with `lim xcode version`. Mise itself and Limrun's runtime helpers are part of the image.

`NODE_BINARY` points to the Node selected by mise in both runs and builds. For React Native and Expo builds, Limrun rewrites `ios/.xcode.env.local` with that binary and the selected tool PATH so Xcode phases use the same tools as dependency installation. Do not copy a client Node path into this file. The selected Java and Flutter installations also provide `JAVA_HOME` and `FLUTTER_ROOT`.
...
`mise install` adds a version without changing the selection. `mise use` installs and records an explicit sandbox override for later operations in `.limrun-runtime-mise.toml`; it takes precedence over project tool requests. `lim xcode use node@24` replaces that override with a Node 24 selection. A running operation keeps its environment snapshot. The next run or build picks up a version change; use `mise exec` explicitly when a child command in the same operation needs the new version immediately.

Image installations stay outside the workspace cache. Mise selects among compatible image and user installations, including restored user versions. User-installed mise tools live under `.limbuild-sandbox/home/.mise/` and are included when your cache paths cover that directory. Existing cache publication rules still apply: running a command alone does not make a workspace eligible; a successful managed build is required. Dependency installs rerun when the resolved tool versions change. Switching Xcode still invalidates the workspace cache. Mise gem wrappers contain absolute paths: after a cache restore moves the sandbox home, reinstall an affected user gem explicitly, for example `lim xcode run -- mise install --force bundler`. Image gem wrappers are packaged to support relocation.
...
Once the build is accepted, the CLI prints the instance ID, a Console link, and the exact command to retrieve its logs. Pass `--json` for `instanceId`, `execId`, `consoleUrl`, and `logsCommand`. If a webhook is configured, the output also includes `webhookUrl`.

`--webhook-url` is optional; add it to receive the terminal result automatically. `--inactivity-timeout` skips the cached Xcode target and creates a fresh instance with the requested lifecycle, so it cannot be combined with `--id`. Active builds continually report activity; the short timeout begins expiring only after build and upload work stops. The inactivity controller checks about every 15 seconds, so a `3s` timeout usually tears the instance down 3 to 18 seconds after its last activity rather than at exactly three seconds.
...
Read the latest build on the remembered instance, or follow it to completion. No exec ID is required:
...
`logs` prints a snapshot of the latest build, including one still running. `--follow` replays its output and waits for that build to finish. Stopping `--follow` stops observation; the remote build keeps running. A snapshot with status `RUNNING` does not mean the build succeeded.

The CLI first requests the daemon's `latest` retained stream. If the daemon or instance returns 404, the CLI lists persisted logs and downloads the newest build's log from S3/R2. Other errors are reported without substituting an older log. Persisted logs remain readable after instance deletion, and reading logs never creates a replacement instance.

Use `--id <instance-id>` to choose another instance. To keep inspecting a particular detached build even after a newer one starts, copy the `logsCommand` printed when it detached. An exec ID is optional:

```bash
lim xcode logs build-1776140344112378000 --id <instance-id>
```

The SDK can replay or follow a build while its daemon remains available:
...
`observeBuildLogs('latest')` reads only the daemon's retained stream, which can include a completed build. It does not fall back to S3/R2. Omit `follow` for a snapshot, or replace `'latest'` with the exec ID returned by `detach()` to observe a specific build.

To retrieve persisted logs from the SDK, list the completed builds and download the selected record's `downloadUrl`:

```ts
const completedBuilds = await lim.xcodeInstances.listBuildLogs(xcodeInstance.metadata.id);
```

This list remains available after the instance is deleted. For an iOS-backed build, use the attached Xcode sandbox ID rather than the simulator ID.
...
Open the Xcode instance's **Build logs** page in the console to see cache
restoration and publication in a separate **Cache activity** section. Your builds
stay in **Build activity**. Each cache entry shows its
outcome, duration, and readable progress messages, with copy and download controls. Publication
runs when the instance terminates; its duration counts the publication work,
not the time the instance spent waiting to terminate.

Completed cache logs remain available after the build finishes or the instance
terminates, subject to the same retention policy as build logs. Live cache status
and elapsed time appear above the history only while a cache operation is running.
Completed durations stay in **Cache activity**. Older operations without
persisted logs cannot be recovered.

The existing `GET /v1/xcode_instances/{id}/build_logs` endpoint includes cache
records with IDs starting with `cache-restore-` or `cache-publish-`. Their
`status` is the terminal cache phase in uppercase; `startedAt`, `finishedAt`,
and `buildDurationMs` describe the cache operation. The log upload itself is
excluded from the duration.

The CLI and console show the same cache transcript, including the transferred
archive size and operation duration. The size describes the compressed archive,
not the expanded workspace. `GET /v1/xcode_instances/{id}/cache` exposes the
transcript as `restore.log` and `save.log`; when following its event stream,
print only newly appended lines. Older server versions may omit these fields.
