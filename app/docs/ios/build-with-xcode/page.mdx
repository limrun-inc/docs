...
A sandbox builds with its node's default Xcode unless you bind it to another installed major. Xcode 27 (beta) is available beside the default; pass the major to any build entry point, or switch an existing sandbox explicitly:

<CodeGroup labels={["CLI","TypeScript"]}>
```bash
lim xcode list-xcode                      # versions the sandbox can build with, and the current one
lim xcode build . --xcode-version 27     # also on create, test and rbe
lim xcode set-xcode 27                    # switch the remembered sandbox
lim xcode get                             # shows "Xcode: 27.0 (27A5252f)"
```
...
The version is a property of the sandbox, not of the command: once switched, every later build uses it until you switch again. Every build prints the Xcode it runs with (`Building with Xcode 27.0 (27A5252f)`).
...
- The switch is refused while a build, command, sync, or `lim xcode rbe` stack is running; stop them first (`lim xcode rbe --stop`).
- Asking for a major the node does not have fails with the available list. Only majors are selectable; the minor is whatever the fleet carries for that major.
