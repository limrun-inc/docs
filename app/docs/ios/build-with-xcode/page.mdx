...
Each `lim xcode run` and `lim xcode build` resolves `mise env --json` once at the start. Dependency installation, project generation, builds and their child processes use that environment. Tool selection requires a sandbox image with mise support. Set a project preference and install later changes explicitly:
...
For mise tools, `use` saves the requested names and versions unchanged in the highest-precedence existing `mise.toml`, `.mise.toml`, `mise.local.toml`, or `.mise.local.toml` in the current directory, creating `mise.toml` when none exists. It syncs the project and shows the selected versions. The sandbox applies Limrun's major/minor compatibility rules. For example, `lim xcode use node@24.5.0` writes `node = "24.5.0"` locally and selects Node 24 remotely. The first sandbox operation with a project mise file runs `mise install` once. Later changes require `lim xcode tools install`. `--cwd apps/mobile` selects a directory inside the sync root; `--id` selects a sandbox. The command rewrites the selected TOML file, preserving its configuration values but not comments or formatting.

Project tool requests take precedence over the detected package-manager major and Limrun's image defaults. Deeper project files override parent files within the synced directory. Personal mise configuration on the client, including `~/.config/mise/config.toml`, is not read or forwarded.
...
Ruby, Python, Go, Flutter, Dart, and tools below version 1 retain `major.minor`; other numeric versions retain their major. Automatic resolution leaves your client files unchanged and ignores `mise.lock`. Only the tool declarations are imported automatically; client tasks, environment, hooks and settings are not imported. `latest` keeps its mise meaning and opts out of a fixed compatibility line. Other custom selectors should be used through an explicit sandbox mise command.
...
`mise install` adds a version without changing the selection. `mise use` installs and records an explicit sandbox override for later operations in `.limrun-runtime-mise.toml`; it takes precedence over project tool requests. `lim xcode use node@24` clears that tool's override in the selected directory and returns to the compatibility-line policy. A running operation keeps its environment snapshot. The next run or build picks up a version change; use `mise exec` explicitly when a child command in the same operation needs the new version immediately.
