...
A signed build can go straight from the build machine to App Store Connect. Pass your App Store Connect API key alongside the signing flags and the freshly signed IPA uploads to TestFlight as part of the build, no downloading, no Transporter, no separate upload step. Like signing, there is no extra toggle: passing the `asc` flags is the intent.
...
// testflight.state: 'accepted' | 'processing' | 'failed'
```
...
Your API key travels with the build request over TLS, is held in memory only for the duration of the build, and is never written to disk on the build machine.

### One-time App Store Connect setup

Three account-side settings make TestFlight delivery fully hands-off. You set them once per app:

1. **Create an API key with the Developer role** in App Store Connect under Users and Access, then Integrations, then Team Keys. Developer is the least-privileged role that can upload builds; don't hand the build a broader key than it needs. Download the `.p8` once and note the Key ID and Issuer ID.
2. **Answer the encryption question at build time** by setting `ITSAppUsesNonExemptEncryption` to `NO` in your Info.plist (for Expo apps: `ios.config.usesNonExemptEncryption: false` in `app.json`). Without it, every build stalls in App Store Connect with "Missing Compliance" until someone answers manually. If your app uses non-exempt encryption, answer accordingly in App Store Connect instead.
3. **Enable automatic distribution on an internal beta group** when creating the group in TestFlight. Members then receive every new build with no per-build assignment. The setting is create-only in Apple's API, so if your existing group lacks it, create a new group with it enabled.
