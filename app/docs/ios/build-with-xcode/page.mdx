...
Once the build is accepted, the CLI prints the instance ID, the console page where the build's progress and logs appear, and the webhook target. Pass `--json` to get the same summary as one JSON object with `instanceId`, `execId`, `consoleUrl`, and `webhookUrl` fields, ready for scripts to parse.
...
A signed build can go straight from the Xcode sandbox to App Store Connect. Pass `--upload-to-appstore` with your App Store Connect API key alongside the signing flags, and the freshly signed IPA becomes available for TestFlight or App Store distribution with no downloading, Transporter, or separate upload step.
