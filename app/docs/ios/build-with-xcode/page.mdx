...
Cloud signing requires a team API key, its issuer ID, and a `--team-id` that matches the key's Apple Developer team. For `release-testing`, `ad-hoc`, and `app-store-connect`, the key must be an Admin key or have **Access to cloud-managed distribution certificates** enabled in App Store Connect.
