...
1. **Create an API key with the Developer role** in App Store Connect under Users and Access, then Integrations, then Team Keys. Developer is the least-privileged role that can upload builds; don't hand the build a broader key than it needs. Download the `.p8` once and note the Key ID and Issuer ID. The key is team-level: one key serves all your apps.
