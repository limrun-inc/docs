...
Each `lim xcode run` and `lim xcode build` resolves `mise env --json` once at the start. Dependency installation, project generation, builds and their child processes use that environment. Tool selection requires a sandbox image with mise support. Set a project preference, then install any missing versions:
...
For mise tools, `use` saves compatibility lines in the highest-precedence existing `mise.toml`, `.mise.toml`, `mise.local.toml`, or `.mise.local.toml` in the current directory, creating `mise.toml` when none exists. It syncs the project and shows the selected versions. It does not install tools; run `lim xcode run -- mise install` when a selected version is missing. `--cwd apps/mobile` selects a directory inside the sync root; `--id` selects a sandbox. The command rewrites the selected TOML file, preserving its configuration values but not comments or formatting.

Use `lim xcode use --global node@24` for personal defaults. The CLI and TypeScript Xcode client read `[tools]` from the client global mise file, normally `~/.config/mise/config.toml`. `MISE_GLOBAL_CONFIG_FILE` and `XDG_CONFIG_HOME` override that location. Project tool requests take precedence over personal defaults, followed by the project's detected package-manager major and Limrun's defaults. Deeper project files override parent files within the synced directory. Global defaults are captured when the Xcode client is created.
...
| Tools | Default line |
|---|---|
| Node.js, npm, npx | Node 22; npm and npx ship with Node |
...
Install additional tools and compatibility lines explicitly with `lim xcode run -- mise install`. Select another Java vendor with a request such as `java@corretto-17`. Upstream support for macOS and the selected runtime still applies. The managed CocoaPods resolver uses the selected CocoaPods tool; a project `Gemfile` does not select its gems.
