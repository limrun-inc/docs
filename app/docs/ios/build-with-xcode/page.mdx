...
The sandbox includes mise and Homebrew for project tools. Their downloads and installed tools stay in this synced workspace's sandbox home and remain available to later commands:
...
Homebrew formulae and trusted third-party taps are supported. Homebrew itself is pinned by the sandbox image, so `brew update` is disabled; casks and services are unavailable. The workspace uses a custom Homebrew prefix instead of the host's `/opt/homebrew`, so formulae whose bottles require the default prefix may not install.
...
Cloud signing works for device SDK builds: `--sdk iphoneos` and `--sdk watchos`. It requires a team API key, its issuer ID, and a `--team-id` that matches the key's Apple Developer team. For `release-testing` and `app-store-connect`, the key must be an Admin key or have **Access to cloud-managed distribution certificates** enabled in App Store Connect.
