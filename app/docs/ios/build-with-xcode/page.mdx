...
A sandbox builds with its node's default Xcode (26.4 today) unless you select another installed version. The fleet carries one released (GA) Xcode per major and, while Apple is seeding one, one beta: today 26.4 GA, 27.0 GA, and 27.1 beta. Two selectors cover them:
...
Pick one per workspace (your git repo, or a `lim set-workspace-dir` assignment), the way a version manager remembers a version:

<CodeGroup labels={["CLI","TypeScript"]}>
```bash
lim xcode version list                    # Select is the value to type, Channel is ga or beta; * marks the one in use
lim xcode use xcode@27                    # prefer the Xcode 27 GA here; switches the remembered sandbox now
lim xcode build .                         # builds with 27, so do test, rbe and new sandboxes
lim xcode version                         # "27.0 (27A266a)"
...
for (const x of installed) console.log(x.version, x.channel); // "26.4 ga", "27.0 ga", "27.1 beta"
...
`lim xcode version list` prints one row per installed Xcode: the bound marker, the Select value (`26`, `27`, `27.1`), the channel, and the full version (`27.1 beta (27A9269)`). `--quiet` prints only the Select values, one per line.

`lim xcode use xcode@27` is equivalent to `lim xcode version set 27`: it remembers the workspace selector and switches the existing sandbox. With no sandbox, it saves the preference for the next one. You can combine requests, for example `lim xcode use xcode@27 node@24`; only the other tools go into mise. `--cwd` applies only to mise tools; `--workspace` chooses the Limrun workspace preference.

The sandbox itself has one Xcode selected at a time, and the workspace preference wins: when a build finds the sandbox on another Xcode (a colleague switched it, or a fresh sandbox came up on the node default), it says so and switches before building. Every build prints the Xcode it runs with (`Building with Xcode 27.1 (27A9269)`).

- Switching Xcode invalidates the build cache produced with the other version, so the next build starts cold. For the persistent build cache (`--cache-key`), use a separate key per Xcode lane, for example `myapp-27` and `myapp-27.1`: archives are stored per key, and a restore under a different Xcode is wiped.
...
- Asking for a version the node does not have fails with the available list, and `lim xcode use xcode@<version>` does not record a preference for it.
...
- When a beta becomes GA, the release replaces the beta under the same major.minor selector, with one cold build. The bare major moves to the new release when the fleet retires the older GA.
