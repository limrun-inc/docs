...
Each `lim xcode run` and `lim xcode build` resolves `mise env --json` once at the start and passes that environment to its commands. Version selection does not install tools. Set a project preference, then install any missing versions:
