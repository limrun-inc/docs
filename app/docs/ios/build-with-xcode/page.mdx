...
Each `lim xcode run` and `lim xcode build` resolves `mise env --json` once at the start. Dependency installation, project generation, builds and their child processes use that environment. Tool selection requires a sandbox image with mise support. Sync your project, then select tools in the sandbox:
...
`lim xcode use` selects tools in an existing sandbox without editing local files or syncing the project. Sync the project first. Mise installs a requested version if needed; `lim xcode tools install` installs selections from synced project files. Use `--cwd apps/mobile` for a nested project and `--id` to choose a sandbox. Explicit sandbox selections take precedence over project requests and apply to subsequent commands.
