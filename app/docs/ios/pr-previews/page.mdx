...
Inputs: `project-path`, `project`, `workspace`, `scheme`, `sdk`, `bazel-target`, `api-key`, `github-token`. Outputs: `preview-url` and `asset-name`.
...
The same Action handles Bazel workspaces: when `project-path` is a Bazel workspace root (has `MODULE.bazel` or `WORKSPACE` directly in it), it builds with [remote build execution](/docs/ios/build-with-bazel) instead of xcodebuild and uploads the built app as the preview asset. Set `bazel-target` (e.g. `//App`) when the workspace has more than one app target; single-app workspaces are inferred.
...
<Columns cols={2}>
  ...
  <Card title="Run XCTest suites" icon="flask-conical" href="/docs/ios/test-with-xctest">
    Add `lim xcode test` to the same workflow to gate previews on your test suite.
  </Card>
</Columns>
