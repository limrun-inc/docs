...
- Generated a `.limrun/` directory in your workspace with the `--config=limrun` Bazel configuration matched to the sandbox's Xcode version, and wired a `try-import` into your `.bazelrc`. Pass `--xcode-version 27` to build with another installed Xcode version (`27` is that major's GA release, `27.1` selects the beta); see [Choose the Xcode version](/docs/ios/build-with-xcode#choose-the-xcode-version).
...
TTLs are optional Go durations (`24h`, `30m`); each upload without one pushes the asset's expiry to 14 days from that upload. Anyone in your organization can then open the build in a browser at `https://console.limrun.com/preview?asset=preview/my-app&platform=ios`.
...
- **Re-run after an Xcode change.** The generated config pins the sandbox's Xcode version; after a fleet upgrade or a `--xcode-version` switch, `lim xcode rbe --stop` followed by `lim xcode rbe` refreshes it.
