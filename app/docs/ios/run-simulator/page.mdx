...
The simulator runs remotely, but the app inside it sometimes needs to reach a service that runs on your side: a mock backend, a test runner, or a local Expo dev server. Declare each TCP destination and start one tunnel:
...
The app keeps using literal `localhost:8000`. The tunnel connects it to `localhost:8000` on the machine running the client. A localhost declaration also captures the exact loopback aliases `127.0.0.1`, `::1`, and `::ffff:127.0.0.1`. Literal IP routes work the same way for non-loopback addresses.

The initial release supports TCP and up to ten exact host-and-port routes. Ports
1–65535 are accepted except port 53. Hostnames other than `localhost`, CIDRs,
...
tunnel. Its route list is immutable: to add or remove a destination, stop the
tunnel and start it again with the complete route list. A WebSocket loss closes
its connections, so start a new tunnel to recover. An open idle tunnel does not
count as instance activity and does not prevent the inactivity timeout.

If the app attempts a route while its local service is stopped, only that
connection fails; the tunnel and its other routes stay active. JSON status
...
value in the route, `TUNNEL_URL`, and Expo's `--port`.
