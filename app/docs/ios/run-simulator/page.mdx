...
The newer [destination tunnel](/docs/platform/network-tunnel) (`lim ios tunnel`) covers the same direction without the 57090-57099 port mapping: the app connects to `localhost:3000` and reaches your machine's `localhost:3000`. Prefer it when the app's code already targets a fixed local address; the reverse tunnel below remains useful when you want an explicit device-facing listener address to hand to the app.
