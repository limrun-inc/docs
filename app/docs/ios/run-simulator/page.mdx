...
If your app stores login or session state in the iOS keychain, save that state after signing in and restore it into later simulators. The default asset name is `keychain/login.tar.gz`.
...
The command creates a `Keychain` asset, generates an encryption key, and prints the matching restore command. Save that key with your test secrets, then restore into another simulator:
