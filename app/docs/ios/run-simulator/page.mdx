...
its connections, so start a new tunnel to recover.
...
`lim ios reverse` remains available for legacy workflows that already choose a simulator-facing port in the reserved range. New workflows should use the destination tunnel above. The direction is simulator to your service.
