...
When Metro is already running, the last two steps collapse into one command. `lim ios reverse --detach` starts the tunnel in a background session daemon and prints only a URL, so it composes with command substitution; with `--scheme` that URL is the dev-client deep link, encoding included. `lim xcode build` prints this exact command after a Debug Expo build:

```bash
lim ios open-url "$(lim ios reverse 57090:57090 --detach --scheme <scheme>)"
```

The detached tunnel lives until you stop the session daemon with `lim session stop` or the instance ends.
