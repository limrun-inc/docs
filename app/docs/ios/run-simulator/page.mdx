...
When Metro is already running, the last two steps collapse into one command: `--open` opens the URL on the simulator as soon as the tunnel is ready, substituting a literal `{host}` with the simulator-facing host, so the command needs nothing you don't know upfront. `lim xcode build` prints this exact command after a Debug Expo build:

```bash
lim ios reverse 57090:57090 --open "<scheme>://expo-development-client/?url=http%3A%2F%2F{host}%3A57090"
```
