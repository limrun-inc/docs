...
- **Idle tunnels don't keep the instance alive.** Only traffic that actually flows through a tunnel counts as instance activity. An open but quiet tunnel lets the instance's [inactivity timeout](#configure-the-instance) fire as usual, so agents that leave tunnels behind stop accruing instance time.
