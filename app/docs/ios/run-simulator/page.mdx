...
console.log(tunnel.bindings);
// Each binding maps one declared route to a simulator-facing host and port.

const status = await client.getTunnelStatus();
await client.stopTunnel(tunnel.tunnelId);
```

`localhost` resolves on the machine running the client. The app connects to the `endpoint` returned for that route. In this first release, the app does not use literal `localhost:8000`.
...
lim ios tunnel \
  ...
  --route api.internal:443 \
  --detach

lim ios tunnel status
lim ios tunnel stop
```

Use `--json` when another process needs the tunnel ID or route mappings. One instance accepts one active destination tunnel with up to ten exact routes. A WebSocket loss closes its listeners and connections, so start a new tunnel to recover.

### Expo and Metro

Metro can listen on its normal local port while Expo advertises the endpoint
returned by Limrun:
...
`EXPO_PACKAGER_PROXY_URL` makes Expo use the simulator-facing host and port in
the manifest, bundle URLs, and deep links. Set it inline so it takes precedence
