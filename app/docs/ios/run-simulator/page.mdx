...
`tapElement` taps with a real synthesized touch, so the app reacts exactly as it would to a finger. When the element isn't on screen, it scrolls the list to find it first (a bounded sweep, a few pages down then up) and taps once the content settles; a deep sweep can take several seconds. Pass `{ activate: 'ax' }` (CLI: `--activate ax`) to use an accessibility press instead: no scrolling, no touch events, but it works on elements without a usable frame.
...
`typeText` sends real hardware key events: text delegates fire, and the field's own keyboard behavior applies (a default text field autocapitalizes the first letter, for example). It requires a focused field and returns an error when there is none, so tap the field first. For an exact value without keyboard behavior, use `setElementValue` above. `pressKey` accepts shifted symbols directly (`@`, `#`, `?`, ...) and derives the shift itself.
...
const instance = await lim.iosInstances.create({
  ...
  reuseIfExists: true,
  metadata: {
    labels: { app: 'my-app', session: 'demo', 'limrun.com/zone-override': 'us-east1' },
  },
  spec: {
...
lim ios create --reuse-if-exists \
  --model iphone \
  --label limrun.com/zone-override=us-east1 \
...
| `region` | string | Deprecated: a preference only; may fall back to other regions when full. Pin with the `limrun.com/zone-override` label in `metadata.labels` instead. |
