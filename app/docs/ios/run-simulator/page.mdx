...
The app keeps using literal `localhost:8000`. The tunnel connects it to `localhost:8000` on the machine running the client. Literal IP routes work the same way; other hostnames are not supported in this release.
...
Use `--json` when another process needs the tunnel ID. One instance accepts one active destination tunnel with up to ten exact routes. A WebSocket loss closes its connections, so start a new tunnel to recover.
