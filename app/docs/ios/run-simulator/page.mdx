...
| `signedStreamUrl` | Console-hosted watch URL with the token in the URL fragment; open in a browser without your API key. |
| `mcpUrl` | Per-instance MCP HTTP endpoint. Auth with the per-instance `status.token`, not your org API key. See [MCP](/docs/agents/mcp); for one org-level endpoint that also creates instances, see the [remote MCP server](/docs/agents/remote-mcp). |
...
independently of the hinge angle. For example, `lim ios fold --orientation portrait --id <instance-ID>`
changes orientation without moving the hinge. Use `lim ios fold 0` to close and
`lim ios fold 180` to unfold; there is no separate `lim ios unfold` command.
...
cover and inner display. Click or hold
the frame's Sleep/Wake and volume buttons to control iOS. In 3D, position starts
unlocked. **Lock position** prevents camera rotation while keeping the screen
and hardware buttons active. See [Embed a simulator](/docs/platform/embed-simulator#iphone-duo-frame)
for the UI component and frameless options.

With the TypeScript device client:
...
`inner` for the unfolded display. These calls do not activate a display: fold
or unfold first. A display that is not ready reports an error. Display screenshots
contain JPEG data, including when saved by `lim ios screenshot --display`.
...
display. Both the 2D and 3D viewers support single-finger touch and drag, but not
multi-touch. Rotating the view changes the camera; **Rotate device** changes native orientation. **Laptop view**
...
Hardware `button` values: `home`, `lock`, `side`, `applePay`, `softwareKeyboard`. Native iPhone Duo also supports `volumeUp` and `volumeDown`.
...
For a two-finger gesture, add `x2` and `y2` to each `touchDown`, `touchMove`, and `touchUp`. Each event moves both fingers together. Keep the same finger ordering throughout the gesture and lift both fingers with `touchUp`. Supplying only one of `x2` or `y2` rejects the batch before it runs.

Both fingers use the same coordinate space. Without `screenWidth` and `screenHeight`, coordinates use the device's display dimensions in its current orientation. To use coordinates from a resized image, supply that image's width and height on every touch action. The server scales and rotates both fingers for HID input.

This pinch-out example spreads two fingers apart over 320 milliseconds:
...
The example assumes portrait orientation and an existing `client`. The local type adds the optional fields for SDK releases whose TypeScript definitions predate this feature; no SDK runtime update is needed. One or two fingers are supported; omit both `x2` and `y2` for a single-finger gesture.

On the signaling WebSocket, this payload spreads two fingers in an explicit 400 by 800 coordinate space:
...
```json
{
  ...
  "id": "pinch-zoom-in",
  "actions": [
    { "type": "touchDown", "x": 180, "y": 400, "x2": 220, "y2": 400, "screenWidth": 400, "screenHeight": 800 },
    { "type": "wait", "durationMs": 80 },
    { "type": "touchMove", "x": 160, "y": 400, "x2": 240, "y2": 400, "screenWidth": 400, "screenHeight": 800 },
    { "type": "wait", "durationMs": 80 },
    { "type": "touchMove", "x": 140, "y": 400, "x2": 260, "y2": 400, "screenWidth": 400, "screenHeight": 800 },
    { "type": "wait", "durationMs": 80 },
    { "type": "touchMove", "x": 120, "y": 400, "x2": 280, "y2": 400, "screenWidth": 400, "screenHeight": 800 },
    { "type": "wait", "durationMs": 80 },
    { "type": "touchMove", "x": 100, "y": 400, "x2": 300, "y2": 400, "screenWidth": 400, "screenHeight": 800 },
    { "type": "touchUp", "x": 100, "y": 400, "x2": 300, "y2": 400, "screenWidth": 400, "screenHeight": 800 }
  ]
}
```

`x` and `y` identify the first finger; `x2` and `y2` identify the second. The `id` correlates the response with this request. Pass only the `actions` array to `client.performActions`, or save that array as JSON and use `lim ios perform --file actions.json`. To zoom out, start with the fingers apart and move them toward each other before releasing both.
...
Play a video file as the simulator's camera. Use this for QR-code scanning, document capture, video-call UIs, or any `AVCaptureSession`-driven flow that needs deterministic frames. The file is uploaded to the instance and replaces the live browser camera until cleared; apps see it through their regular capture pipeline.
...
The simulator runs remotely, but the app inside it sometimes needs to reach a service that runs on your side: a mock backend, a test runner, a local Expo dev server, or an API that only your machine or VPN can resolve. Declare each destination as a selector and start one tunnel:
...
Domain selectors are intercepted transparently on the instance and dialed from
the machine running the client, whether or not the name resolves on public DNS,
so your DNS, `/etc/hosts`, and VPN apply. Write an
exact name (`api.corp.example`) or a `*.` wildcard (`*.staging.example`);
wildcards are label-bound, so `*.staging.example` covers `a.staging.example`
but not `staging.example` itself. `localhost` and IP literals are not domains;
declare those as `host:port` selectors.

A tunnel carries TCP only and accepts up to ten exact `host:port` selectors and
up to 64 domain selectors. Ports 1 to 65535 are accepted except port 53. CIDRs and
UDP are not supported. Apps that resolve DNS themselves over HTTPS (DoH) bypass
domain interception; declare the resolved IP instead.

A tunnel captures connections the app opens after the tunnel is ready. An app
that reached a destination before that keeps its open connections and can reuse
them outside the tunnel, so start the tunnel before you launch the app or open
the page, or relaunch the app once the tunnel is ready.
...
HTTP and HTTPS through the tunnel are inspected by default. Each completed
request is printed as one summary line (in the tunnel log file when detached)
and streamed live to the console's network panel. `--har <path>` writes the
traffic as HAR 1.2 with bodies, and `--persist` uploads a network log as a
session artifact when the tunnel stops or the instance terminates (default
lifetime 3 days, `--ttl <seconds>` up to 30 days). HTTPS is decoded with a
certificate authority the simulator trusts, so apps that pin certificates fail
through inspected selectors: leave the pinned host out of the selectors, or
pass `--no-inspect` to keep TLS end to end. From the SDK, pass
`inspection: { enabled: false }` to `startTunnel` for the same effect.
...
By default, instances are an iPhone in a nearby region with a server-applied inactivity timeout. To pick a different model, keep the instance inside one jurisdiction, pre-install an app, or attach an Xcode build sandbox, pass `spec.*` at create time:
...
| `jurisdiction` | `'us' \| 'eu' \| 'as'` | Hard constraint: schedule only inside this jurisdiction; creation fails when no region there has capacity. TypeScript SDK and CLI today; the Python and Go SDKs expose only `region`. |
| `region` | string | Deprecated. A region preference (e.g. `us-east1`), not a hard pin: the request overflows to other regions by proximity when the preferred ones are full. Otherwise scheduled by clues + availability. |
...
| `clues` | array | Scheduling hints. Use `ClientIP` for the end user's IP, or the REST API's `ClientLocation` with `{ latitude, longitude }` for coordinates. `ClientLocation` takes precedence. See [client location clues](/docs/reference/sdk#client-location-clues). |
...
<Columns cols={2}>
  ...
  <Card title="Run XCTest suites" icon="flask-conical" href="/docs/ios/test-with-xctest">
    Run your unit and UI test targets on this simulator with `lim xcode test`.
  </Card>
  <Card title="Remote MCP server" icon="plug" href="/docs/agents/remote-mcp">
    Let an agent create and drive simulators through one org-level MCP endpoint.
  </Card>
</Columns>
