...
| `clues` | array | Scheduling hints. `{ kind: 'ClientIP', clientIp }` supplies the end user's IP for geographic placement. If missing or empty, Limrun uses Cloudflare's `CF-Connecting-IP` header. Region preferences, jurisdiction, availability, and organization routing policies still apply. |
...
<Note>
  **For platform integrators:** pass the end user's IP as a `ClientIP` clue (`clues: [{ kind: 'ClientIP', clientIp: '203.0.113.42' }]`). Without it, Cloudflare's header identifies your backend's connection rather than the user's browser. See [Instance placement](/docs/reference/sdk#instance-placement) for fallback behavior.
</Note>
