...
App bundle assets can carry the app's identity as metadata: the title users see on iOS (`displayName`), `bundleIdentifier`, `shortVersion` (CFBundleShortVersionString), `buildVersion` (CFBundleVersion), and the app's primary URL scheme (`deeplink`, e.g. `myapp` for `myapp://` links). All fields are optional; consumers such as over-the-air installation and the Console use whichever are present, and `assets.list` / `assets.get` return them alongside a time-limited `signedIconUrl` when an icon was extracted.

Builds through `lim xcode build --upload` record the metadata automatically from the built bundle, including the app icon. For bundles built elsewhere, set it at upload time:

<CodeGroup labels={["CLI","TypeScript"]}>
```bash
lim asset push ./MyApp.ipa \
  --upload-option "displayName=My App" \
  --upload-option bundleIdentifier=com.example.app \
  --upload-option shortVersion=1.2.3 \
  --upload-option buildVersion=42 \
...
On re-upload, provided fields overwrite the recorded ones and omitted fields are left unchanged.
