...
| `kind` | `App` (default) for installable binaries, `Keychain` for keychain archives used by Xcode build sandboxes. |
...
| `platform` | Limits which platform can install the asset. Useful when the same logical name (e.g. `acme/my-app-v1.2.3`) covers both an iOS sim build and an Android APK. Responses also include `os`, a deprecated alias that mirrors this field. |
| `expiresAt` | When set, the time after which the asset (bytes and record) is deleted automatically. See [Asset expiry](#asset-expiry-ttl). |
...
Stored assets are always files: `kind: 'App'` installables or `kind: 'Keychain'` archives. But `spec.initialAssets` accepts another flavour of entry, `kind: 'Configuration'`, which carries an instance-level setting instead of pointing at Asset Storage. That value exists only on the [`initialAsset` shape](#pre-install-on-boot); an `Asset` record is never `Configuration`.
...
Every upload path accepts an optional `ttl` as a Go duration string (`--ttl` on `lim asset push`, `ttl` on `getOrUpload`/`getOrCreate`, `--upload-ttl` on `lim xcode build` and `lim gradle build`). When set, the asset is deleted (bytes and record) that long after the upload; the minimum is `1m` and `1d` is invalid Go syntax, use `24h`.

```bash
lim asset push ./build/MyApp.app.tar.gz -n my-app-v1.2.3.tar.gz --ttl 720h
```

The default depends on the upload path:

- **Direct uploads** (`lim asset push`, `getOrUpload`, `getOrCreate`) never expire unless you pass a `ttl`. Re-uploading under the same name keeps the recorded expiry; pass a `ttl` on the re-upload to change it.
- **Build-product uploads** (`lim xcode build --upload`, `lim gradle build --upload`, [RBE uploads](/docs/ios/build-with-bazel)) default to **14 days**: each build upload without an explicit `--upload-ttl` pushes the asset's expiry to 14 days from that upload, so actively rebuilt assets stay alive while abandoned build artifacts get swept.
...
```json
{
  ...
  "status": {
    ...
    "signedStreamUrl":         "https://console.limrun.com/signedStream?url=…#token=lim_...",
    "mcpUrl":                  "https://.../mcp",
...
- **iOS-only (legacy):** `status.sandbox.xcode.url` on instances created by old clients with `spec.sandbox.xcode.enabled`; new clients create the Xcode instance separately and attach the simulator.
...
For the full field reference, see [Status URL surface](/docs/reference/sdk#status-url-surface).

## Working with Expo Go

Limrun maintains a small curated catalog of pre-built apps under the `appstore/` name prefix: Expo Go, which lets React Native users on your platform open a Snack or a local Expo project on a remote device without uploading anything, and the Maestro XCTest runner used by [the Maestro integration](/docs/testing/maestro).
...
| `appstore/maestro-ios-runner-2.5.1.tar.gz` | iOS | Maestro XCTest runner for [Maestro flows](/docs/testing/maestro) |
...
The `platform` field on each `Asset` lets you further split a tenant's namespace by platform when the same logical name covers both an iOS sim build and an Android APK:
...
const universal   = list.filter((a) => !a.platform);
```

Integrations written before the field was named `platform` may still read `os`; it is returned as a deprecated alias with the same value.
