...
interface Asset {
  ...
  expiresAt?:        string;                           // when the asset is deleted; defaults to 14 days after creation
...
| `expiresAt` | The time after which the asset (bytes and record) is deleted automatically. See [Asset expiry](#asset-expiry-ttl). |
...
When omitted, a **newly created asset expires 14 days after creation**. Re-uploading under the same name keeps the asset's recorded expiry: pass a `ttl` on the re-upload to push the expiry out from that moment.

```bash
lim asset push ./build/MyApp.app.tar.gz -n my-app-v1.2.3.tar.gz --ttl 720h
```

For assets you want to keep long-term, set a generous `ttl` explicitly and refresh it on re-uploads.
