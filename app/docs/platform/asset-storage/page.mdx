...
`state` cycles through `'unknown' \| 'creating' \| 'assigned' \| 'ready' \| 'terminated'`.

<Note>
  `spec.inactivityTimeout` currently round-trips as `"&Duration{Duration:10m0s,}"`. This looks like a backend serialization bug, the field should probably come back as a plain duration string. Until it's fixed, rely on the value you sent rather than parsing what the API returns.
</Note>
