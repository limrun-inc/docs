...
This page describes how a coding agent platform should implement the UX for their customers to install their apps on Limrun instances.

For how to get platform users to connect and control to these instances, see [Embed the simulator](/docs/platform/embed-simulator).
        ...
        InitialAssets: []limrun.IosInstanceNewParamsSpecInitialAsset{
            {
                Kind:       "App",
                Source:     "AssetName",
                AssetName:  limrun.String("my-app-v1.2.3.tar.gz"),
                LaunchMode: "ForegroundIfRunning",
            },
            {
                Kind:   "App",
                Source: "URL",
                URL:    limrun.String("https://example.com/builds/123.tar.gz"),
            },
            {
                Kind:    "App",
                Source:  "AssetID",
                AssetID: limrun.String("asset_01j..."),
            },
        },
    },
})
        ...
        InitialAssets: []limrun.AndroidInstanceNewParamsSpecInitialAsset{
            {
                Kind:      "App",
                Source:    "AssetName",
                AssetName: limrun.String("my-app-v1.2.3.apk"),
            },
            {
                Kind:   "App",
                Source: "URL",
                URL:    limrun.String("https://example.com/builds/123.apk"),
            },
            {
                Kind:    "App",
                Source:  "AssetID",
...
`sendAsset(url, timeoutMs?)` triggers a server-side download + `pm install`. The default client-side timeout is **120 s** (verified at `typescript-sdk/src/instance-client.ts:1038`); pass a higher value if your APK is large enough that the install can run past two minutes.
...
`state` cycles through `'unknown' \| 'creating' \| 'assigned' \| 'ready' \| 'terminated'` (verified at `typescript-sdk/src/resources/android-instances.ts:96`). `spec.inactivityTimeout` is returned as a Go duration string (`"10m0s"`, not the `"10m"` you pass in).
