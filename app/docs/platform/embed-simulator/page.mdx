...
└────────────────┬────────────────┘    └────────────────┬────────────────┘
                 ...
                 ▼                                     │
          Limrun instance ◀─────────────────────────────┘
          (status.endpointWebSocketUrl, status.streamToken)
```

Three parts of the system:

- **End user's browser.** Renders `<RemoteControl />`, sees only a per-instance WebSocket URL and the instance's stream token. Never sees your API key or the instance's control token.
...
- **Limrun control plane.** Provisions the instance and returns two tokens scoped to it: `status.token` for control and `status.streamToken` for the screen.

### Authentication

The client browser never sees your API key. It gets a WebSocket URL and `instance.status.streamToken`, a token scoped to the screen of just the **one** instance: video, touch, keyboard, screenshots, accessibility inspection, open URL, quit app, camera and microphone sharing. It cannot install apps, run commands, read files or open ADB, so a user who lifts it from the browser cannot drive the simulator beyond what the embed already shows. It has no expiry of its own and stops working when the instance terminates.

`instance.status.token` is the control token. It grants everything on the instance, including the SDK, the CLI and MCP, so keep it on your backend and never send it to the browser.

Android instances return `streamToken` in a following release. Until then, Android embeds keep passing `instance.status.token`.
...
The single contract between your backend and `<RemoteControl />` is the pair `endpointWebSocketUrl + streamToken` from `instance.status`. Every spec field, region clue, label pattern, and lifecycle knob lives on the per-platform pages above.
...
app.post('/sessions/start', async (req, res) => {
  ...
  res.json({
    ...
    token:     instance.status.streamToken,
  });
});
...
async def start(req: Request):
    ...
    return {
        ...
        "token":     instance.status.stream_token,
    }
...
| `token` | string | required | Per-instance bearer. Pass `instance.status.streamToken`; it grants the screen and nothing else. The component appends it to the connection URL as `?token=…`. |
