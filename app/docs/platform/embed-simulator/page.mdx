...
WebGL, the view stays flat. The last screen image remains visible while the next
display prepares its layout. The 3D frame shows the physical hinge
pose, and position starts unlocked. Drag the frame or background, or Alt-drag the
...
For a custom body appearance, pass `duoModelUrl` with a prepared GLB that you are
