...
iOS simulator streams are capped at 30 fps. WebRTC handles network and encoder-load adaptation, so video resolution can change during a session. These changes affect the streamed image, not the simulator's native display geometry or app layout. You don't need an additional resolution controller in your integration.
