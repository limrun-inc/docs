...
- **Scheduling clues.** Pass the end user's IP as a `ClientIP` clue to prefer a region near their browser. Without a nonempty clue, Limrun uses Cloudflare's `CF-Connecting-IP` header, which identifies your backend's connection when your backend creates the instance. Region preferences, jurisdiction, availability, and organization routing policies still apply.
