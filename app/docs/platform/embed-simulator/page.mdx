...
description: "Render a live iOS or Android device inside your web app with `<RemoteControl />` from `@limrun/ui`. Your backend keeps the API key; the browser only sees a per-instance URL and token."
order: 1
icon: "monitor-play"
---
# Embed the simulator
...
This is the path platform integrators take: companies whose end users need a live mobile device on the page. Cloud coding agents that work on iOS or Android. Mobile preview tools. Internal QA dashboards. Same component, same wiring, on every one of those.
...
- **A Limrun API key, kept server-side.** Sign up at [console.limrun.com](https://console.limrun.com), open **Settings → API Keys**, and create a token. See [Authentication](/docs/reference/sdk#authentication) for the full flow.
...
- **Scheduling clues.** Pass the end user's coordinates as a REST API `ClientLocation` clue, or their IP as a `ClientIP` clue, to prefer a nearby region. `ClientLocation` takes precedence when both are supplied. See [client location clues](/docs/reference/sdk#client-location-clues) for the payload and coordinate limits.
- **Labels for tenant scope and reuse.** Labels are free-form key-value pairs which can be used to organise multiple tenants or reuse hot instances. If `reuseIfExists` is set to `true`, the scheduler returns an existing hot instance whose `labels` match the current create call exactly, looked up in the region that handles the call, so a user reopening their tab lands back on the same warm instance.
...
| `showFrame` | boolean | `true` | Draw the device frame. For Duo, `false` hides the frame, hardware buttons, viewer toolbar and 2D/3D toggle. See [iPad and Apple Watch](#ipad-and-apple-watch) for models without built-in frames. |
| `deviceModel` | `'iphone-duo'` | auto-detected | Enable the Duo viewer explicitly. Otherwise the component discovers Duo support from the connection. This does not select the model when creating an instance. |
| `duoModelUrl` | string | built-in model | Optional URL of a prepared custom Duo GLB. See [iPhone Duo frame](#iphone-duo-frame). |
...
| `onCameraDemandChange` | callback | none | Reports when an app opens or closes its camera and includes the browser's actual capture settings after permission or a track change. |
...
These gestures apply to the standard viewer. Duo supports single-finger taps and
drags in both modes, but not multi-touch or mouse-wheel scrolling. In Duo's 3D
view, Alt-drag rotates the view rather than pinching the screen.

Keyboard events forward while the embedded screen is focused. Tap the screen to focus it.

### Camera simulation

When an app opens its front or back virtual camera, `<RemoteControl />` asks
the browser for the matching `user` or `environment` camera and sends that
track to the instance. Switching cameras in the app replaces the WebRTC track
automatically; your embed does not need its own camera toggle.

Facing is a preference rather than a requirement. A facing change restarts
browser capture with an `ideal` constraint. On a laptop with one camera, the
browser naturally returns that camera again while the app still observes the
selected virtual position and its normal front-camera mirroring behavior.

The browser and WebRTC choose the capture resolution. The virtual-camera output
automatically follows each decoded frame's rotated dimensions, including
dimension changes after a camera switch.
...
Camera capture requires HTTPS or `http://localhost`, as required by
`getUserMedia`.

### iOS extended touch area
iOS treats the home-indicator strip as part of the gesture surface. To make swipe-up-from-home, app-switcher, and pull-from-bottom gestures work the same way they do on a real device, `<RemoteControl />` exposes a 60-pixel extended touch area below the visible screen. A drag started inside that strip and moved up enters the iOS gesture system the way it would on hardware. This extended area applies to the standard viewer; on Duo, start the gesture on the visible display.
...
Create an iPhone Duo on your backend by setting `spec.model` to `iphone-duo` in
`lim.iosInstances.create`. The model must be available to your organization.
Return `instance.status.endpointWebSocketUrl` and `instance.status.token` to the
frontend, as in the provisioning example above.

Render the Duo with the same component:
...
`deviceModel` can be omitted when the connection reports native Duo support.
Our 2D frame and procedural 3D model ship with `@limrun/ui`; you do not need to
host a model file. The CLI and device API control the simulator, while the UI
package renders its frame.

When connected to an iPhone Duo instance, the component defaults to a lightweight
2D view with a silver Duo frame, hinge spine and physical buttons. Both views share
the fold, hinge-angle and rotation toolbar. Select **3D** to load the
folding frame on demand. Switching back to **2D** releases the 3D renderer while
keeping the simulator connected.

The 2D frame shows the active display flat, with a brief hinge animation when you fold
or unfold it. The animation loads on demand and respects reduced motion; without
WebGL, the view stays flat. When a screen image is available, the animation holds
it briefly while waiting for a frame from the incoming display. The 3D frame shows
the physical hinge pose, and position starts unlocked. Drag the frame or background, or Alt-drag the
screen, to rotate the 3D view. Ordinary screen gestures control iOS. Use Lock position
to prevent rotation. In both modes, the device's Sleep/Wake and volume buttons support
clicking and holding. Hover near an edge to reveal its icons, then click either
the icon or the physical button. The icons follow the device when it rotates.
Camera Control is not yet supported.

Set `showFrame={false}` to show only the active display, without the frame,
hardware buttons, viewer toolbar, or 2D/3D toggle. Touch, drag, and keyboard input
remain active. The video follows native display and orientation changes without
loading a 3D renderer. Use the [device client or CLI](/docs/ios/run-simulator#fold-an-iphone-duo)
to control the hinge and orientation outside the viewer.

For a custom 3D body appearance, pass `duoModelUrl` with a prepared GLB that you are
licensed to serve. Limrun does not bundle a third-party model. The file must follow
the [Duo appearance contract](https://github.com/limrun-inc/typescript-sdk/blob/main/packages/ui/README.md#optional-duo-appearance).
The live simulator screens and fold controls remain active with a custom appearance.
This prop replaces the full 3D body only; the 2D frame and its fold animation keep
...
<Columns cols={2}>
  ...
  <Card title="Run an iOS Simulator" icon="smartphone" href="/docs/ios/run-simulator">
    Backend-side device control: tap by accessibility selector, element-tree, recording, tunnels.
  </Card>
