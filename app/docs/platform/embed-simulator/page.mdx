...
| `cameraResolutionCap` | `'auto' \| '1080p' \| '720p' \| '480p'` | `'auto'` | Preferred browser capture size using standard landscape dimensions. |
...
The virtual-camera output automatically follows each decoded frame's rotated
dimensions, including changes after a camera switch. Use `cameraResolutionCap`
to choose the preferred browser capture size:
