...
| `cameraResolutionCap` | `'auto' \| '1080p' \| '720p' \| '480p'` | `'auto'` | Preferred browser capture size using standard landscape dimensions. This is independent from the virtual-camera output aspect. |
| `cameraAspect` | `'auto' \| '16:9' \| '4:3' \| '1:1' \| '9:16'` | `'auto'` | Virtual-camera output sizing. Auto follows the browser track; an explicit value pins the simulator or emulator camera aspect. |
...
By default, the virtual camera follows each decoded frame's rotated dimensions,
including changes after a camera switch. Set `cameraAspect` to pin a host-side
output shape. The host scales and center-crops frames into that shape without
changing the browser's capture constraints:

```tsx
<RemoteControl
  url={session.webrtcUrl}
  token={session.token}
  cameraResolutionCap="720p"
  cameraAspect="9:16"
...
Use `cameraAspect="auto"` again to resume track-following. Camera capture
requires HTTPS or `http://localhost`, as required by `getUserMedia`.
