...
<Steps>
  ...
  <Step title="Prepare and store Apple credentials">
    ...
    The App Store Connect team key must be an Admin key or have **Access to cloud-managed distribution certificates** enabled. Limrun uses it for both cloud signing and upload. Reuse a valid stored key when the user reconnects.
  </Step>
</Steps>
...
2. Writes the `.p8` bytes to a temporary directory with owner-only permissions.
3. Starts a release build for the `iphoneos` SDK.
4. Passes the team ID and App Store Connect key to the CLI for cloud signing.
...
Add `--scheme <scheme>` when the project needs one. Cloud signing requires a team API key and its issuer ID. Spawn the CLI with an argument array instead of constructing a shell command.
