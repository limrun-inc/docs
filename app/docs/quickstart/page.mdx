...
<Steps>
  ...
  <Step title="Build on a cloud Mac">
    ...
    The first run provisions an Xcode build sandbox for you, syncs the project, and runs `xcodebuild` remotely. Build logs stream back as they happen. For multi-scheme projects, add `--scheme <name>` and either `--workspace <path>` or `--project <path>`.
  </Step>

  <Step title="Run it on a simulator">
    Create a simulator attached to the sandbox. The attach installs and launches the build immediately, and every later successful build reinstalls automatically.

    ```bash
    lim ios create --attach --reuse-if-exists --label project=quickstart
    ```

    Open the **Signed Stream URL** from the output in any browser to watch the simulator live. The URL has its own token embedded, so no sign-in is needed. Your app's first screen appears a few seconds after the attach.
...
When you're done, delete the instances. Without an ID, each delete removes the last instance of that type the CLI created for you (the pointer is in `~/.lim/last-instances.json`).
...
<Columns cols={2}>
  ...
  <Card title="Remote MCP server" icon="plug" href="/docs/agents/remote-mcp">
    Point any MCP client at mcp.limrun.com and let the agent create and drive devices itself.
  </Card>
  <Card title="Run XCTest suites" icon="flask-conical" href="/docs/ios/test-with-xctest">
    Build a scheme's test targets on remote Xcode and run them on a cloud simulator with streamed results.
  </Card>
</Columns>
