...
<Note>
  `reuseIfExists` returns an instance whose labels match exactly, looked up in the region that handles the create. Placement follows your location and capacity, so a create from a different network can land in another region, miss the match, and start a fresh instance. Give each CI job or dev session its own label set. Pass `jurisdiction` (`us`, `eu`, or `as`; TypeScript SDK and CLI) when the instance must stay in one geography; it narrows placement but does not pin one region. `spec.region` is deprecated and only a preference.
</Note>
...
<Columns cols={2}>
  ...
  <Card title="Run XCTest suites" href="/docs/ios/test-with-xctest">
    Native XCTest targets on remote Xcode and simulators, no WebDriver layer.
  </Card>
