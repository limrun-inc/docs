...
Code examples default to TypeScript with `webdriverio`. Instance-control snippets have Go and Python tabs where the SDK supports it (Python is control-plane only). The Appium capabilities themselves are wire-format and work with any Appium client (`appium-python-client`, Java `java-client`, Ruby, C#).
...
The snippet below is in `webdriverio`. If your test code is in Python (`appium-python-client`), Java (`java-client`), Ruby, or C#, set the same `appium:*` keys on your client's capabilities object the way you would any other Appium capability.
