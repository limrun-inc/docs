...
Every Playwright WebSocket connection requires this instance token, supplied
as `Authorization: Bearer <token>` or `?token=<token>`. Missing or invalid
tokens receive HTTP 401; a valid token for another instance receives HTTP 403.
